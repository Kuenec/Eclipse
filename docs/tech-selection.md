...
| **AndroidManifest (binary XML)** | **`axmldecoder`** (manifest); `apk-info` if full ARSC needed | ✅ | Maintained | `axmldecoder` is lightweight, parses just the manifest (package id, launcher Activity, sdk levels, native-lib name). `apk-info` is the heavier "full AXML+ARSC, ~10× faster, hardened" option if we need resources too. |
