...
- **2026-06-11 — 🚀 THE `Java_*` DISCOVERY GAP IS CLOSED (general reflection-based RegisterNatives); the Roblox lifecycle
  ...
  in-scope, gate-green):** after each pre-loaded lib, enumerate ALL its exported `Java_*` symbols
  (`engine::LoadedEngine::java_native_exports`), demangle → `(class, method)` (`src/loader/jni_mangle.rs`, 8 tests),
  reflect each declaring class's declared `native` methods for their full JNI signatures, and `register_native_methods`
  to the export address (`src/loader/jni_register.rs::register_all_preloaded_natives`, best-effort + per-class/per-method
  `with_local_frame`). **VALIDATED on the real v2.721.1108 boot:** bound **466 libroblox natives (60 classes) + 144
  zstd natives (10 classes)**; the lifecycle went **step 3 `Application.onCreate` ✅ → step 4 `Activity.createMainActivity`
  ✅ → step 5 `ActivitySplash.onCreate`** (Roblox's REAL launcher Activity now runs). A small curated table is kept as a
  fallback if reflection binds nothing. **New main-thread frontier = an ATL framework METHOD gap** (same category as the
  Build field, NOT a native): `NoSuchMethodError android.net.NetworkRequest$Builder.addCapability(int)` (the jobqueue lib
  in `ActivitySplash.onCreate`) — fix by extending the patched-`api-impl.jar` overlay (add the method to ATL's
  `NetworkRequest`, like the Build field). SQLite `nativeOpen` + `java.time` BootstrapMethodError + Firebase Parcel remain
  (worker-thread / deeper). **NOTE: the Build/framework patches only apply with `ECLIPSE_ANDROID_FRAMEWORK_DIR=~/.cache/
  eclipse/framework-patched`** (the user's bare `./eclipse run` does not set it → field/method gaps reappear; a durable
  auto-provision is open). Gate: **504 unit + 4 integration + 2 doctests** (+8 demangler), fmt/clippy `-D warnings`/release
  all 0-warning. Still NO render. Not committed (owner's session instruction). Detail: §6 (2026-06-11 discovery-gap).
- **2026-06-11 — MAIN-THREAD `onCreate` ADVANCED past a missing-Java-FIELD blocker; new frontier = the libroblox `Java_*`
  discovery gap.** Live-boot evidence corrected the "frontier = SQLite" framing: step 3 `Application.onCreate` actually
  aborted on the MAIN thread with `NoSuchFieldError android.os.Build.SUPPORTED_64_BIT_ABIS` (ATL's `api-impl.jar` Build omits
  it); SQLite's `nativeOpen` fails only on a parallel WorkManager worker thread, so SQLite alone would not advance
  `eclipse run`. **Fixed** by a patched ATL `api-impl.jar` (the field is a missing FIELD, not a native → `RegisterNatives`
  can't help → framework-jar fix): a MULTIDEX overlay (`classes.dex` = patched `Build` with `SUPPORTED_{32,64}_BIT_ABIS`,
  wins; `classes2.dex` = original) built with only `javac`+`dx`+`jar`, provisioned via `ECLIPSE_ANDROID_FRAMEWORK_DIR`
  (no root) — a **dev-host framework artifact** (like vendored ART), reproducible via
  `~/.cache/eclipse/build-field-patch/patch-build-field.sh`. **VERIFIED:** `NoSuchFieldError` GONE, `onCreate` advanced
  307 → 319. **New main-thread blocker = the pre-loaded-lib `Java_*` discovery gap** (`JNIAAssetManagerSetup.initNative`,
  `JNILoggingProtocol.*`, `zstd…recommendedDInSize` — all libroblox `Java_*` natives ART can't `dlsym` because Eclipse
  mmap-pre-loads libroblox, not via an ART `dlopen` handle). Fixing it unblocks a whole class at once. **Deterministic core
  DONE + gate-green:** `src/loader/jni_mangle.rs` — a pure, total (`#![forbid(unsafe_code)]`) JNI symbol DEMANGLER
  (`Java_<class>_<method>[__sig]` → `(class, method)`, escape-aware: `_1`/`_2`/`_3`/`_0XXXX`, nested-class `$`, overload
  long form), 8 unit tests incl. the 3 real blocking symbols. **NEXT = the live wiring** (main-loop): enumerate each
  pre-loaded lib's `Java_*` exports (Eclipse has name→addr in `engine`/`resolve`), demangle, reflect each class's declared
  native method for the full JNI signature, and `register_native_methods` to the export address — careful work against the
  **redesigned `jni` 0.22.4 API** (`Env`/`NativeMethod`/`JNIStr`, study first). SQLite stays a real worker-thread frontier
  for after. Still NO render. Gate: **504 unit + 4 integration + 2 doctests** (+8 demangler), all 0-warning. Not committed
  (owner's session instruction). Detail: §6 (2026-06-11 Build-field).
- **2026-06-11 — 🎉 THE #1 ENGINE-LOAD WALL IS CROSSED (the apkenv `System.loadLibrary` SIGSEGV):** Eclipse now
  intercepts ART's `Runtime.nativeLoad` and CONSULTS its pre-load registry — a pre-loaded soname
  (`engine::is_preloaded`) is reported already-loaded (`null` = success) so it **skips the apkenv shim linker**,
  while every other lib is DELEGATED to ART's real `JavaVMExt::LoadNativeLibrary` (dlsym'd, called via a clean-room
  C++ shim `src/loader/native_load_shim.cpp`) so it loads through ART's normal path (libraries_ / JNI_OnLoad /
  `Java_*` discovery) UNCHANGED. **VALIDATED on the user's v2.721.1108 APK:** `Runtime.nativeLoad: already
  pre-loaded … apkenv skipped soname="libzstd-jni-1.5.7-6.so"`, the `unknown reloc type 18`/apkenv SIGSEGV is GONE,
  `libwolfssljni` loads via delegation, and **Roblox's `Application.onCreate` runs its full content-provider init**
  (ShellConfig / AppAssets / FileProvider / **MLKit / Firebase** / OkHttp). NEW post-wall frontiers (normal): (a) the
  pre-loaded lazy-native **discovery gap** (zstd's `Java_*` aren't in ART's `libraries_` → `recommendedDInSize()`
  UnsatisfiedLinkError — register them with ART next), (b) the missing framework native
  `AssetManager.getResourcePackageName(int)` (Firebase; Eclipse's `arsc` has `package_name`), (c) an exit-teardown
  SIGSEGV on the error path (§8 class — engine workers live at `main` return; vanishes once the lifecycle succeeds).
  Done MAIN-LOOP with explicit owner authorization (the gated region). Gate clean: **487 unit + 4 integration + 2
  doctests**, fmt/clippy `-D warnings`/release all 0-warning. Full detail: §6 (2026-06-11) + `docs/libroblox-init-run.md`
  §9–§11. **Honest status: Roblox does NOT render yet** — the load+init+onCreate path is now open to content-provider
  init; the discovery-loop frontiers above are next.
- **2026-06-11 — POST-WALL DISCOVERY LOOP: Roblox's `Application.onCreate` now reads its compression dictionary fully
  + creates ALL content providers; frontier advanced to SQLite.** After the nativeLoad wall, the dev-host run drove
  the discovery loop forward (each a benign `android.*` native, gate-green): (1) **`AssetManager.getResourcePackageName(int)`**
  + (2) **`getResourceIdentifier(String,String,String)`** — bound to Eclipse's `apk::arsc` (the latter needed a NEW
  total, bounded `arsc::ResTable::find_resource_id` reverse lookup name→id); this got Firebase's `onCreate` past its
  resource lookups and **advanced the lifecycle step 2 → step 3 (`Application.onCreate`)**. (3) The **asset-stream
  subsystem** — new `src/framework/asset_registry.rs` (jlong generational slab holding the asset bytes + cursor,
  mirroring `xml_registry`) + 6 natives (`openAsset`/`readAsset`/`seekAsset`/`getAssetLength`/`getAssetRemainingLength`/
  `destroyAsset`) backed by Eclipse's `src/apk` reader. Two bugs found+fixed on the real run: a **double-`assets/`
  prefix** (ATL passes `openAsset` the FULL `assets/…` path, not a relative one) and ATL's **`readAsset(J[BJJ)I`**
  (long off/len, not the classic `(J[BII)I`); the read cycle is registered PER-NATIVE best-effort (ATL declares
  `getAssetLength`/`destroyAsset`/`readAsset` etc.) and `readAsset` is bounded to `b[]`'s real length so a
  `set_region` overflow can't leave a pending JNI exception (which ATL surfaced as `IOException`). **Result (real
  v2.721.1108 run):** onCreate reads the 1.2 MB `shared_compression_dictionaries/*.dict` fully (148 × 8192-byte
  `readAsset`s) and creates ShellConfig/AppAssets/MlKit/Firebase/**MobileAds** providers. **NEW frontier = SQLite:**
  `android.database.sqlite.SQLiteConnection.nativeOpen(String,int,String,boolean,boolean)` (Roblox opens a DB during
  onCreate) — a LARGE subsystem (the full `SQLiteConnection`/`CursorWindow` native surface, ~25 natives, needs a real
  SQLite backing). The pre-loaded-lib `Java_*` discovery gap (zstd `recommendedDInSize`) + the exit-teardown SIGSEGV
  remain. Gate clean: **496 unit + 4 integration + 2 doctests**, fmt/clippy `-D warnings`/release all 0-warning. Detail:
  §6 (2026-06-11 post-wall loop).
