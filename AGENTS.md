...
- **2026-06-14 — ✅✅ SHADER-PACK REJECT ROOT-CAUSED AND FIXED — IT WAS A FIRST-PARTY ECLIPSE BUG, NOT "internal/gated/off-policy". THE ENGINE NOW LOADS THE SHADER PACK, CREATES ITS RenderView, AND REACHES THE LuaApp LANDING SCREEN. ⇐ START HERE NEXT SESSION. (commit `ac5feba`)**
  ...
  **NEXT:** verify visible frames actually present to the winit window (the render job runs + swapchain is up + DataModel is APP_READY, but confirm pixels — fresh boot + screenshot). Then the next layer is the LuaApp landing/login UI rendering (the 403 heartbeats are auth-gated, separate). Audit any OTHER `-1`/stub NDK natives that a fall-back-less caller might treat as fatal.
