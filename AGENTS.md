...
- **2026-07-10 — 🌉 WEB-ENGINE MILESTONE M4 DONE: JS bridge + cookies + honest UA via the additive protocol-v2
  ...
  CookieManager is native-backed against ONE session-scoped private in-memory cookie store (the fabricated
  Boolean.TRUE is gone), the recorded "GDPR VIOLATION" UA is replaced by an honest deliberate Chromium-149 UA in
  BOTH units, and the `javascript:`-println full-URL leak channel is overlay-patched BEFORE `javascript:` URLs
  become secret-bearing — IMPLEMENTED + gate green + dev-host `__webview-test` run VERIFIED 2026-07-10 (full
  record in §6 2026-07-10 🌉 and the M4 Status block in `docs/web-engine-plan.md`) ⇐ START-HERE-NEXT: MILESTONE
  M5 of `docs/web-engine-plan.md` — distro-agnostic hardening + packaging (runtime detect-don't-assume with
  actionable errors: DT_NEEDED host-lib probe, sandbox-mode selection [SUID chrome-sandbox / unprivileged userns
  / loud documented degradation per the owner ruling], Wayland AND X11, GPU vs bundled SwiftShader; packaging:
  ...
  skewed peer answers HelloAck{its version} and closes, latching the M3 honest one-shot-WARN no-op degradation),
  new wire types ct 0x11–0x14 (BridgeRegister/BridgeResult/EvaluateJsForResult/CookieSetForResult) + ht
  0x89–0x8B (BridgeCall/EvaluateJsResult/CookieSetResult) with caps enforced BEFORE allocation, v1 layouts
  FROZEN + the v1 round-trip pin untouched (+ a separate v2 pin over the 7 new messages); bridge/eval/cookie
  payloads OPAQUE to the helper (never parsed, logged as lengths/counts only). (2) The async-Promise bridge —
  the DOCUMENTED DIVERGENCE from AOSP's synchronous @JavascriptInterface contract (CEF's only cross-process
  JS↔native primitive is async; the sandboxed renderer main thread is NEVER blocked; a page expecting a
  synchronous return gets a Promise — re-validate against RBHybridWebView's real shape at M6, plan open question
  #3); annotation-gated reflection (the new RUNTIME-retained overlay @JavascriptInterface class is the security
  gate) with ARITY-BASED overload resolution per the Chromium gin java-bridge reference, and a PULL-model
  inventory (the renderer requests per main-frame context — CEF drops early pushes, live-observed). (3) ONE
  session-scoped private in-memory RequestContext cookie store (empty cache_path, persist_session_cookies=0)
  shared by every browser + every cookie op — the .ROBLOSECURITY handoff lands in the store the challenge
  WebView reads; getCookie = bounded 5 s blocking round-trip with honest-empty degrade. (4) The honest
  deliberate UA (`Mozilla/5.0 (X11; Linux x86_64) … Chrome/149.0.0.0 … Eclipse-WebView/149.0.6`) byte-matched
  helper-side (`build_settings()`) + overlay WebSettings (getUserAgentString AND getDefaultUserAgent), pinned in
  both units — genuinely Chromium 149 on Linux, deliberately identifying, never impersonating a device. (5)
  WebView natives `bound=3` → **`bound=5`** (native_evaluateJavascript + native_addJavascriptInterface) + NEW
  CookieManager registrar **`bound=6`**; the @JavascriptInterface annotation + inert EclipseBridgeProbe test
  class added via javac (classes.dex); WebView/WebSettings shadowed into classes2.dex behind the established
  anchor/pristine/back-check §-guards. (6) Post-review threading hardening — a dedicated `eclipse-webview-upcall`
  thread now owns ALL app-facing JNI (internalLoadChanged, bridge reflect-invokes, every ValueCallback, the
  deferred closed-view bridge drop; the socket reader is fully JNI-free) and era-gated drains give exactly-once
  honest-failure callback delivery on every loss path incl. the close+re-drive corner. MEASURED verify (dev-host
  main thread, 2026-07-10, `/tmp/eclipse-webview-m4-test.log`, 52 lines): `timeout 180 ./target/release/eclipse
  __webview-test` → EXIT=0 + marker `WebView engine pipeline OK: internalLoadChanged upcalls 2/2 (state 0 @
  150ms, state 3 @ 150ms, http 200), frame 1024x768 237 distinct pixels, bridge round-trip OK,
  evaluateJavascript OK, honest UA OK, cookie set/get OK, cookie callback OK, ViewClosed, helper exit 0,
  bound=5` + the CookieManager `bound=6` registration line; handshake logged
  `engine=cef/149.0.6+g0d0eeb6+chromium-149.0.7827.201 protocol=2`; helper resolved to the release binary; the
  driven page is an OFFLINE first-party loopback page (real http origin — the 237-pixel ink census is the
  solid-color test page, not the roblox.com range; the live-page dependency moves to M6); privacy greps 0
  full-URL-shaped lines (scheme+host only) / 0 ROBLOSECURITY-shaped strings (bridge method names, cookie values,
  UA and eval results deliberately never logged); /proc orphan scan clean (an earlier identical run on the
  pre-close-out tree also passed EXIT=0, state 0/3 @ 200 ms). Review ledger: first review 7 alleged / 7
  CONFIRMED / ALL FIXED (incl. the reader-reentrancy self-deadlock class — a synchronous getCookie from any
  upcall stalled the io thread 5 s and returned a wrong empty); post-fix review 4 alleged — 2 confirmed + fixed
  (never-driven-view bridge leak; stale queued ViewClosedDrain vs close+re-drive), 1 refuted-with-mechanism
  (Eclipse's own per-view jobject global pins every constructed View — the GC-finalize trigger is unreachable in
  production), 1 skeptic-died → re-adjudicated CONFIRMED by the close-out pass (the reader's ViewClosed bridge
  drop did hidden scoped JNI attach/detach per global) + fixed (moved to the upcall thread, era-gated); stale
  protocol-v1 strings fixed tree-wide (the handshake log binds the negotiated `protocol=2`). Gate: root
  fmt/build/clippy -D warnings/test/release ALL clean — **629 unit + 6 integ + 2 doctest, ZERO SKIP** (was
  613+6+2 at M3; the `__webview-test` guard drove the full pipeline through REAL ART); helper crate **23 + 15**
  clean (was 17+11); root Cargo.toml/Cargo.lock stay cef-free. Carried: the M3 carried-to-M6 list unchanged (ATL
  2-arg onPageStarted reconciliation; driven-loads-only contract; centered-fallback rect no-input;
  challenge-rect/ResizeView at M6); TWO new owner-flagged unconfirmed nuances (bounded, recorded not fixed) — an
  in-flight eval callback of a finalized (notify-path) view whose result raced the finalize is held until the
  helper-teardown drain, and the pre-existing pending_bridges inventory-loss nuance on close+re-drive; M6 must
  overlay-validate the javascript:-URL leak fix against the real challenge flow.
- **2026-07-03 — 🔗 WEB-ENGINE MILESTONE M3 DONE: main-process wiring at the recorded seams — the two validated
  ...
  `javascript:`-println full-URL leak channel BEFORE `javascript:` URLs become secret-bearing) — DONE 2026-07-10
  (see the 🌉 bullet above; the live pointer moved to MILESTONE M5).** What shipped
...
- **2026-06-14 — ✅✅✅ HOST INPUT IS FORWARDED — CLICKS REACH ROBLOX AND DRIVE THE UI. Tapping "Sign In" navigated the app Landing → LoginV2. ⇐ START HERE NEXT SESSION.**
  ...
  2. **`android.webkit.ValueCallback` class→interface (overlay).** ATL shipped it as an empty `public class`; Roblox's `ql.b implements ValueCallback` → `IncompatibleClassChangeError` at `CookieProtocol.<init>` (auth/cookie path) → one of the message storms. Patched to the AOSP interface (`onReceiveValue(T)`) in `tools/framework-overlay/src/android/webkit/ValueCallback.java` (classes.dex first-dex-wins), with a `patch-framework.sh` grep regression guard. (More auth-path gaps remain — `CookieManager.setCookie(String,String,ValueCallback)`, `JobParameters.getNetwork()`, `View.nativeIsAttachedToWindow`/`getWindowVisibleDisplayFrame` — they no longer FREEZE the loop thanks to the bounded pump, but they break login NETWORKING; see follow-ups.)
  3. **Engine-mode pointer path (`src/graphics.rs` + `src/framework.rs`).** Post-handoff `self.renderer` is `None` (dropped at `graphics.rs:353`), so the old `handle_primary_press`/`release` early-returned and the hit-test targeted Eclipse's own view tree (can't resolve engine-drawn Lua UI). New branch on `self.handed_off`: `engine_primary_press`/`engine_primary_release` dispatch a raw-coordinate `MotionEvent` (constant downTime across DOWN/UP) to the engine's `RBXSurfaceView` via `dispatch_touch_to_engine_surface` → **`onTouchEventInternal(event, false)`** (ATL's REAL touch driver, View.java:1133, which invokes the engine's `OnTouchListener`). NOT `onTouchEvent`/`dispatchTouchEvent` — both are `{ return false; }` stubs in ATL (View.java:1147/2062). Plus a per-event `feed_winit_input_to_loopers` wake (engine input threads park in `ALooper_pollOnce`). Env-gated dev diagnostic `ECLIPSE_SYNTHETIC_ENGINE_TAP="x,y"` fires one real DOWN+UP ~6 s post-handoff (mirrors `maybe_synthetic_tap`).
  **LIVE-CONFIRMED** (`/tmp/boot_oti.log`, patched framework): synthetic tap at (400,390)=Sign In → `onDataModelNotification type:APP_READY data:LoginV2` + `ActivityNativeMain onAppReady: LoginV2` (Landing→Login nav); the owner's subsequent clicks all log `consumed=true`. Gate green (fmt/clippy/567 unit + 4 integ + 2 doctest/release).
  NOTE: `RBXSurfaceView` reports `getWidth/getHeight == 0` (ATL never lays out the content view) — harmless for the listener path (the engine's `OnTouchListener` forwards by coordinate, not view bounds), but a real `View.layout(0,0,w,h)` is a correctness follow-up.
  ✅ **POINTER DRAG + KEYBOARD now also forwarded (2026-06-14, same input subsystem):** (a) `MotionAction::Move` + `engine_pointer_move` dispatch `ACTION_MOVE` to the RBXSurfaceView while the primary button is held (drag-scroll, sliders), reusing the press downTime. (b) Keyboard: `WindowEvent::KeyboardInput` → `dispatch_key_to_engine_surface` builds a `KeyEvent` (6-arg ctor `(JJIIII)V`), **sets the package-private `unicodeValue` field** (ATL `getUnicodeChar()` returns it verbatim; public ctors leave it 0 — without this nothing types), and calls **`RBXSurfaceView.dispatchKeyEvent`** (`(Landroid/view/KeyEvent;)Z`). A live boot RESOLVED the entry point: `onKeyUp` is ABSENT on the view and base `onKeyDown` is a `return false` stub, so `dispatchKeyEvent` (the route ATL's own GTK key driver uses) is correct — both DOWN+UP dispatch with NO error (`consumed=false` only because the synthetic tap hit a button, not a focused field). winit→Android keycode map covers A–Z/0–9/space/./@/-/backspace/enter/arrows + a running META_* modifier bitmask; the typed char rides on `unicodeValue` (so even an unmapped printable types). Env diag `ECLIPSE_SYNTHETIC_KEY` fires a hardcoded 'a'. **Keystrokes are NOT logged (only consumed/pressed) — never log credentials.** Regression guards: `key_action_codes_match_public_android_constants`, `key_event_dispatch_descriptors_are_the_public_android_api`, `winit_keycode_maps_credential_keys_to_android_keycodes`.
  🔎 **LOGIN-FIELD TYPING — FULL PATH TRACED (live boot + a self-contained two-stage synthetic: env `ECLIPSE_SYNTHETIC_ENGINE_TAP="400,390"` taps Sign In → LoginV2, then `ECLIPSE_SYNTHETIC_TYPE="400,171:robloxtest"` taps the username field + types).** Confirmed: the synthetic TAP genuinely FOCUSES the field (screenshot shows the blue focus border — touch fully works end-to-end), but keys to `RBXSurfaceView` are `consumed=false` and the field stays empty. ROOT MECHANISM: Roblox's login field is an **Android `EditText` driven by `com.roblox.client.RbxKeyboard`** (NOT the SurfaceView). On focus the engine (via `EngineJavaCallbackWrapper` → `RbxKeyboard.i/l`) configures that EditText, hitting a CASCADE of ATL gaps — each a clean no-op fix:
    1. ✅ `AutofillManager.cancel()` (missing) — added (overlay smali). Unblocked `RbxKeyboard.i`.
    2. ✅ `View.setAutofillHints(String[])` + `setImportantForAutofill(int)` (missing) — added (overlay smali). `RbxKeyboard.l` advanced `Source:23 → :191`.
    3. ❌ **`EditText.native_getText(long)` is UNBOUND** (`UnsatisfiedLinkError`) — `RbxKeyboard.l` calls `EditText.getText()`. THIS is the current frontier.
  ✅✅ **EDITTEXT TEXT-BACKING SUBSYSTEM IMPLEMENTED — LOGIN TYPING FUNCTIONALLY WORKS.** Typing into the login field now REACHES THE ENGINE: a live boot shows the "Next" button go from disabled (dim) to ENABLED (bright blue) after typing — the engine validated the typed username. Components shipped: (1) `EditText.native_getText(long)→String` bound — the framework's first **value-returning** native (`JString<'local>` IS `Default` → null, so `with_env(...).resolve::<LogErrorAndDefault>()` works; no newtype needed). It also records the polled widget as `ACTIVE_TEXT_FIELD` — Roblox's `RbxKeyboard` polls the FOCUSED field's `getText()`, so that call IS Eclipse's focus signal. (2) `type_into_active_text_field(vm, unicode, backspace)` (graphics `engine_key` routes printable/backspace DOWN here, falling back to the surface for in-game keys) edits the active field's `view_registry` text via the pure, unit-tested `apply_text_edit` (correct per-keystroke `onTextChanged` delta), then (3) `fire_text_watchers` fires the retained `TextWatcher`'s `onTextChanged` + `afterTextChanged` via JNI — snapshotting the (non-`Clone`) watcher `Global`s' raw refs UNDER the registry lock, firing OUTSIDE it (a watcher re-enters via getText → deadlock otherwise). The earlier RbxKeyboard cascade gaps (`AutofillManager.cancel`, `View.setAutofillHints/setImportantForAutofill`) are fixed in the overlay. Self-validated by a 3-stage synthetic (`ECLIPSE_SYNTHETIC_TYPE="x,y:text"`: tap Sign In → focus field → type). Keystrokes are NEVER logged. Regression guards: `apply_text_edit_computes_android_ontextchanged_delta`, the key/value-native descriptor tests.
  ⚠️ **REMAINING (visual polish, NOT functional):** the typed text does not yet VISUALLY render IN the field box. Confirmed via a `RUST_LOG=eclipse::framework=debug` boot that the engine NEVER `setText`s the field back after typing → it is NOT a sync/clear loop; it is a pure DISPLAY/RENDER gap (the engine receives + validates the text — Next enables — but its visible field is drawn from a source that doesn't reflect the input). Most likely the engine relies on the Android `EditText`'s own native text draw (a separate Android view on real Android), which Eclipse's no-GTK framework does not composite over the engine's GL surface. Functional login (type → Next → proceed) works; making the typed text VISIBLE is a follow-up (engine-display investigation / compositing the EditText text). The `RBXSurfaceView.dispatchKeyEvent` route stays correct for IN-GAME keys.
  🔬 **VISIBLE-TEXT — ENGINE TEXT-BRIDGE SURFACE FULLY MAPPED + EXERCISED; RENDER GAP IS ARCHITECTURAL (2026-06-14).** A JNI-reflection diagnostic (`framework::reflect_engine_input_methods`, env `ECLIPSE_REFLECT_INPUT`, fired ~8s post-handoff from graphics) dumped the engine's COMPLETE input/text bridge surface — `com.roblox.engine.jni.NativeGLInterface` (text/keyboard) + `NativeInputInterface` (pointer/gamepad/sensor). The text bridges are exactly: `nativePassText(long,String,boolean,int)`, `syncTextboxTextAndCursorPosition2(String,int)`, `updateKeyboardSize(boolean,int,int,int,int)`, `nativePassKeyEvent(boolean,int,int,boolean)`, `nativeReturnPressedFromOnScreenKeyboard(long)`, `nativeGetTextBoxInfo()→NativeTextBoxInfo`. **`type_into_active_text_field` now forwards each keystroke through the engine's OWN protocol** — `sync_engine_textbox` calls `updateKeyboardSize(true,…)` (signal keyboard-shown) → `nativePassText(0,text,false,cursor)` (IME commit; the `long` is an eventTime, NOT a handle — see below — so 0 is crash-safe) → `syncTextboxTextAndCursorPosition2(text,cursor)`, and `pass_key_event_to_engine` sends `nativePassKeyEvent` DOWN+UP per char (`android_keycode_for` maps the codepoint → `KEYCODE_*`+metaState; unit-tested). **EXHAUSTIVE RESULT:** every bridge is delivered cleanly (0 throws, no crash, `handled=true`), yet the focused login field renders empty and the presented frame is **byte-identical** (`grim` PNGs, 17708 B) across `nativePassText`, `syncTextbox`, `updateKeyboardSize`, AND `nativePassKeyEvent` boots — i.e. NO engine text/input bridge visually renders the typed text on Eclipse. **Decisive structural evidence:** `nativeGetTextBoxInfo()` returns NON-null (an active textbox session exists) but its `NativeTextBoxInfo` carries ONLY geometry/style (`x,y,width,height,font,fontSize,textColor,textInputType,multiline,returnKeyType,alignment` — a fresh instance per call) — **no `text`, no `cursor`, no handle field**. So (a) the `long` in `nativePassText`/`nativeReturnPressed` cannot be a textbox pointer Java derefs (Java is never handed one), and (b) the info object is EditText-overlay positioning/styling config, not a text carrier. ⇒ The visible login text on Android is almost certainly a native `EditText` the OS composites over the GL surface (configured via `NativeTextBoxInfo`); Eclipse's GL-surface-only presentation does NOT composite Android views post-handoff, so the text is invisible even though the engine receives + validates it. Pinning the engine's exact internal render trigger would need RE of `libroblox` (cyber-safeguard OFF-LIMITS). **Visible typing therefore needs an architectural phase, not a bridge call:** (a) an Android-View overlay compositor (draw the focused `EditText`'s text over the engine surface), or (b) engine-IME-composition emulation (RE-blocked). Input FORWARDING is complete (text reaches the engine via its real protocol); only the VISUAL composite remains. Regression guard for the new path: `android_keycode_for_maps_credential_keys`.
  🖼️ **VISIBLE-TEXT OVERLAY — VULKAN PRESENT-PATH INJECTION, SEAM PROVEN (2026-06-14, Step A).** The owner approved building the in-policy overlay. A live boot CONFIRMED the engine renders + presents via **Vulkan** (Mode 6: `Vulkan Device: NVIDIA …`, `swapchain images 3 … size 800x600`, present `vkQueuePresentKHR`) — so the overlay is a MangoHud/Steam-overlay-style injection into the engine's OWN Vulkan present (NOT a GLES quad / Wayland subsurface — Eclipse dropped its renderer + the engine owns the single `wl_surface`; Wayland can't subsurface across winit's connection). New module `src/loader/vk_overlay.rs` extends Eclipse's existing tier-0 Vulkan-WSI interposition ([`vulkan_wsi`]): `eclipse_vk_get_instance_proc_addr` now also hands back Eclipse's `vkCreateDevice` + `vkGetDeviceProcAddr`; the latter hands back `vkQueuePresentKHR`/`vkCreateSwapchainKHR`/`vkGetSwapchainImagesKHR`. **Step A (landed, live-validated):** capture the engine's `VkDevice`, swapchain handle+format (37 = `R8G8B8A8_UNORM`)+extent (800×600)+the 3 images, and intercept `vkQueuePresentKHR` (pass-through; logs ONCE that the seam is armed with `device_set/swapchain_set/images=3/image_index` all present). Boot proof: interposition fires at frame rate, all objects captured, ENGINE RENDERS NORMALLY (no crash/corruption) — the whole approach is de-risked. **Step B (LANDED + VERIFIED — see §6 2026-06-14):** the overlay now READ-MODIFY-WRITEs the swapchain image — reads the focused field's rect, CPU-rasterizes the field text onto it (`ab_glyph`, white glyphs over the engine's bg/border), writes it back. A self-contained field PROBE (`ECLIPSE_VK_PROBE`, ASCII ink-sparkline — the dev host can't screenshot the buried window past Opera's fullscreen) confirmed `ECLIPSE_VK_TEXT_TEST=robloxtest` composites glyph-shaped ink INTO the engine frame (`| ==*@=@#=*##=#…|` total_ink=425), stable, engine alive. The native-render path (composing) was RULED OUT (live but intractably buggy — triples chars + auto-defocuses; fights the engine's protocol, needs off-limits RE). **Step C — DONE, DEFAULT-ON, FULL LOGIN-USERNAME FLOW VERIFIED (2026-06-14, see §6):** visible login typing WORKS and is the default (no env flags). DISPLAY = the overlay (always on; only does GPU work while a field is focused → zero gameplay cost via a fast path) draws the typed text at the field's LIVE geometry (`framework::query_textbox_geometry` reads `NativeTextBoxInfo` x/y/w/h on the main thread → cached for the present-thread overlay). DETECTION = `text_paths` default `w` (the EditText TextWatcher → RbxKeyboard delivers to the engine) — NO composing/nativePassText (those caused the kick-out/tripling). Autonomous proof (a new self-contained PNG screenshot — `encode_png_rgba`, no deps — since `grim` can't reach the buried window): synthetic tapped "Sign in to another account" (400,466) → LoginV2 → focused the username field (400,171) → typed "robloxtest" (`handled=true` ×10, NO defocus) → the field PNG shows "robloxtest" rendered → tapped Next (400,236) → **the engine RESOLVED the username to the real account "RobloxTest/@robloxtest" and ADVANCED to the password step.** Display + detection + advance, all confirmed. (Known follow-up: the per-present readback+writeback slows the framerate while a field is focused — fine for login; optimize to redraw-on-change later. Same flow applies to the password field.)
  ✅ **LOGIN-NETWORK FRAMEWORK GAPS CLOSED (2026-06-14, overlay smali):** `CookieManager.setCookie(String,String,ValueCallback<Boolean>)` (delegates to the 2-arg + reports success to the callback) and `JobParameters.getNetwork()→Network` (returns null, AOSP-valid) added — a boot confirms both `NoSuchMethodError`s are GONE (count 0), render + LoginV2 intact. The auth/cookie + scheduled-network-job paths no longer throw. (Remaining network item: the `http://10.110.101.222:5052` LAN-IP connection timeout — a private-IP Roblox-port connect at the login stage; likely a connectivity/DNS or local-server-config nuance, needs its own look. The `apis.roblox.com` 403s are pre-auth, expected.)
  NEXT layers toward "everything works": the VISIBLE login-text render (the hard in-policy boundary above — engine-display RE [off-policy] or EditText-over-GL compositing); the LAN-IP connectivity nuance; scroll wheel (`ACTION_SCROLL` needs axis values); a SIGTERM/`/tmp` cleanup handler; audio; actual login (needs visible typing + a real account).
- **2026-06-14 — ✅✅ SHADER-PACK REJECT ROOT-CAUSED AND FIXED — IT WAS A FIRST-PARTY ECLIPSE BUG, NOT "internal/gated/off-policy". THE ENGINE NOW LOADS THE SHADER PACK, CREATES ITS RenderView, AND REACHES THE LuaApp LANDING SCREEN. (commit `ac5feba`)**
  **Root cause:** `eclipse_aasset_openfiledescriptor` (`src/loader/native_provider.rs`, the NDK `AAsset_openFileDescriptor`) was a **`-1` stub** on the (wrong) assumption that an in-memory asset has no backing fd so "the caller falls back to `AAsset_getBuffer`". libroblox's shader-pack loader does NOT fall back: for the large **STORED** packs (`vulkan_mobile` ~18 MB, `glsles3` ~9 MB) it uses the **fd/`mmap`** path and treats a `< 0` return as fatal → `Error opening shader pack <name>` → `RenderView is NULL` → no frames. This one stub was the blocker behind EVERY Mode 6 (Vulkan) and Mode 4 (GLES3) failure for the entire investigation.
  **Fix (committed `ac5feba`):** Eclipse already holds each asset's full decompressed bytes in memory (`AssetState.bytes`, byte-exact with the APK entry), so back them with an anonymous in-memory file — `memfd_create` → `ftruncate` → write the bytes → rewind → return its fd + `[0, len)` (the real-Android contract: an fd the caller can `mmap`, not a fake fd). Falls back to the documented `-1` only on a stale handle or a checked syscall failure. Regression guard: the `aasset_openfiledescriptor` test now requires a real fd with byte-exact contents (`fd >= 0`, `out_start == 0`, `out_length == len`, read-back == source) — fails on a regression to the `-1` stub or any wrong-bytes/offset bug. Full gate green (fmt/build/clippy/567 unit + integ/release).
  **LIVE-CONFIRMED on the dev host** (`/tmp/boot_fd.log`, patched framework, EXIT=124 clean 240 s, 1311 lines): `Loaded 3470 shaders from pack vulkan_mobile variant default (17967131 bytes)` (line 1039) · `RenderView created[1]` (line 1007, was NULL) · `Vulkan: swapchain images 3 … size 800x600` · `SurfaceController::RenderJob` running · `onDataModelNotification() type:APP_READY data:Landing` (line 1201) · `LuaAppStarterScript` running. The only remaining errors are HTTP **403** on `user-heartbeats-api/pulse` — expected (no authenticated user session; not logged in), NOT a render failure.
  ⚠️ **THIS SUPERSEDES the entire "shader reject is internal to libroblox / FLog-allowlist-gated / needs RE or server flags / anti-tamper / signature" conclusion below (all the 🔒/🔬/🔑/✅ entries dated 2026-06-14 through the Phase-7 entry).** Those were WRONG. Specifically the strace entry's "**Eclipse does NOT mediate the read … no `AAssetManager`, no asset-serving**" is FALSE: the 2435× merged-APK `openat`s WERE Eclipse's own `eclipse_aassetmanager_open` re-opening the APK per asset (`crate::apk::Apk::open` opens it each call) — Eclipse DID mediate the asset path, and the shader pack specifically went through `AAsset_openFileDescriptor` → `-1` → reject. The bytes were byte-perfect (that part held), but the fd contract was the bug. Lesson: "the engine reads correct bytes therefore Eclipse mediates nothing" was a non-sequitur — the fd PATH, not the byte content, was broken.
  ✅ **VISUAL RENDER CONFIRMED (owner screenshot, 2026-06-14):** the Eclipse window shows the FULL Roblox landing screen — ROBLOX logo, the textured game-thumbnail grid, "Create Account" / "Sign In" buttons, Terms/Privacy links. So pixels genuinely present to the winit window via the engine's own Vulkan/EGL surface (post present-loop handoff). **Roblox is rendering.**
  ✅ **INPUT BLOCKER RESOLVED — see the leading entry above (bounded pump + onTouchEventInternal + ValueCallback).** The renderer-None gate + wrong-target-tree diagnosis here was correct but NOT sufficient; the dominant blocker was the frozen event loop (unbounded `Looper.loop()` under a message storm). Also corrected: the engine's input route is `onTouchEventInternal → OnTouchListener` (NOT `dispatchTouchEvent`/`onTouchEvent`, both ATL `return false` stubs), and `hasSystemFeature(touchscreen)=false` did NOT gate touch (clicks register fine without it). Design from workflow `eclipse-input-forwarding-investigation`.
  🧹 **DEV-HOST CLEANUP FINDING (2026-06-14):** 8 stale `eclipse` boots from prior sessions were still RUNNING (elapsed 45–99 min) — they IGNORED the `timeout --signal=TERM` and held 9 zombie 800×600 windows + 114 MB `/tmp/eclipse-engine-*` each. Killed via SIGKILL. This is the "cleanup-on-exit TODO" biting: Eclipse does not handle SIGTERM (no graceful shutdown), and `/tmp/eclipse-engine-<pid>/libroblox.so` is never removed. Between boots: SIGKILL stale `eclipse` PIDs + `rm -rf /tmp/eclipse-engine-*`. A real fix (SIGTERM handler that exits + `/tmp` cleanup on Drop) is a worthwhile follow-up.
- **2026-06-14 — ❌ RETRACTED: "ACTIVE ANTI-TAMPER" WAS WRONG. The `~libroblox+0x_88a4d0` crash is the engine SIGSEGV-ing on a PENDING JAVA EXCEPTION during the lifecycle, NOT an anti-tamper handler.**
  **How it was disproven (this session, via the file-backed-mapping experiment):** I changed the loader to map `libroblox` PT_LOAD segments FILE-BACKED (workflow-designed, all 572 tests pass, fully reverted afterward — design lives in the saved workflow script `design-file-backed-mapping`). The loader **fully succeeded** (`mapped objects=2 RELATIVE_applied=527214 RELRO_applied=2`, `3427/3427 constructors`, `JNI_OnLoad → 0x10006`, `473 Java_* bound`) — yet the boot still crashed at `~0x_88a4d0`. The log shows WHY: `framework lifecycle step failed step="step 3 Application.onCreate" error=Java exception was thrown`, caused by `com.roblox.client.RobloxApplication.onCreate → Firebase/GMS init → java.io.StreamCorruptedException: invalid stream header: 00000000`. So `0x_88a4d0` = libroblox crashing because a Java exception is pending across a JNI boundary during the lifecycle.
  ⇒ The "4 perturbations → same crash" evidence is REINTERPRETED: ptrace/re-sign/re-zip/cmdline-rewrite each **provoked a Java exception during the lifecycle** (e.g. re-zip → ATL asset/resource parse throws; cmdline change → some Java read throws), and the engine SIGSEGVs on the pending exception. There is NO behavioral evidence of an anti-tamper integrity check. The earlier "anti-tamper / make Eclipse ATL-indistinguishable" direction is WITHDRAWN as unsupported.
  **The shader-pack reject is SEPARATE from this crash** — it occurs in the normal boot that does NOT hit `0x_88a4d0` (reaches render). So the shader reject is still an unexplained in-engine RBXS-parse reject (the real boundary), now WITHOUT any anti-tamper framing.
  ✅ **RESOLVED 2026-06-14 — the "Firebase/GMS onCreate crash on every boot" was NOT a lost WIP or anti-tamper; it was BOOTING WITHOUT THE PATCHED FRAMEWORK.** Root cause: `ECLIPSE_ANDROID_FRAMEWORK_DIR` was UNSET, so the boot used the STOCK ATL framework (`runtime.rs` default `/usr/lib/java/dex/android_translation_layer`) instead of Eclipse's overlay-patched one (`~/.cache/eclipse/framework-patched`). The stock framework lacks the committed overlay patches (`NetworkRequest$Builder.addCapability`, `ActivityManager`, `PowerManager`, etc.), so the boot died early — first the GMS/Firebase `StreamCorruptedException` became fatal, then `NoSuchMethodError: NetworkRequest$Builder.addCapability(I)` / `JobParameters.getNetwork()`. **FIX (durable, 2026-06-14): `eclipse run` now AUTO-DETECTS the patched overlay** — `runtime::find_framework` prefers `ECLIPSE_ANDROID_FRAMEWORK_DIR` > the overlay at `$XDG_CACHE_HOME/eclipse/framework-patched` > stock, and warns (pointing at `patch-framework.sh`) rather than silently booting the broken stock framework. So the env-var export is no longer required (the earlier "always set the env var" runbook invariant was an operator-discipline workaround; this supersedes it — see §6 2026-06-14). Still rebuild the overlay first via `tools/framework-overlay/patch-framework.sh` if `~/.cache/eclipse` was wiped. CONFIRMED on a CLEAN tree at HEAD: `cargo run --release -- run <APK>` (now auto-detecting the overlay — no env var) reaches the full render stage and the Firebase `StreamCorruptedException` is NON-FATAL with the patched framework (degrades gracefully; the Parcel/`readSerializable` overlay-patch idea is MOOT), onCreate completes (no tolerance needed — that experiment was reverted). **UPDATE 2026-06-14 — the "shader-pack reject / RenderView is NULL" boundary is GONE in the current build (that note was STALE).** A fresh boot logs the OPPOSITE: `RenderView created[1]` → `Caps: … Shaders 1 …` → `Vulkan: swapchain images 3 present mode 0 format 37 size 800x600` → **`Loaded 3470 shaders from pack vulkan_mobile variant default (17962007 bytes)`** → `Compiled 1063 shaders in 32 ms` → `setStage: LuaApp` → the React reconciler commits the login UI. Zero render/shader failures in the log. The earlier "Error opening shader pack" was almost certainly the pre-asset-stream state (the shader pack is read from the APK assets, which the now-wired `AssetManager` openAsset/readAsset/openAssetFd natives serve). ⇒ **The render path to the login screen is fully healthy; there is NO outstanding non-login-gated render boundary.** Full-GAME rendering is gated on actual auth (login), which needs the user's real credentials. SECOND GOTCHA fixed: `/tmp` (16 GB tmpfs) filled because each boot stages a 114 MB `libroblox.so` to `/tmp/eclipse-engine-<pid>/` and never cleans it — `rm -rf /tmp/eclipse-engine-*` between boots (an Eclipse cleanup-on-exit TODO).
  File-backed mapping is CORRECT + a perf win (no 111 MB copy) and would make `/proc/self/maps` show `libroblox.so` — but it's NOT a render fix (its anti-tamper premise is retracted) and its faster load shifts Firebase onto the onCreate thread, so do NOT re-land it until the Firebase issue is fixed.
  🔒 **SHADER-REJECT REASON IS DEFINITIVELY GATED (re-tested CLEANLY on the restored dev-host 2026-06-14, no env confound).** The engine reads its local FastFlag override from the host path **`/data/local/tmp/ClientAppSettings.json`** (literal — ATL passes `/data/` through to the host; create with `sudo` — passwordless sudo works on this box; the in-app `files/exe/ClientSettings/` path is NOT it). With the file present + read, FLog overrides (`FLogGraphics/FLogSurfaceController/FLogShaderManager/...=7`) are **DENIED**: `FLog::Graphics` line count stays 83 (== baseline), and the engine logs `getFlagValueFromSettings: 'DFStringAllowedPublicFlags' key not found` → default-deny. Setting `DFStringAllowedPublicFlags` LOCALLY (listing the FLog flags) did NOT help — it is not locally settable (gated by itself). ⇒ The specific shader-pack reject reason CANNOT be read in-policy: it needs either Roblox's server to ship a permissive `DFStringAllowedPublicFlags` (uncontrollable) or RE of `libroblox` (off-policy). This is the hard, confirmed wall — stop re-testing FLog/allowlist; it's settled. (Using Roblox's documented `ClientAppSettings.json` mechanism is in-policy config, not RE/tampering — but it simply doesn't grant the flags.)
  🔬 **STRACE OF THE READ PATH (2026-06-14, render-stage reached at ~line 1191 under a `-e trace=openat,openat2,readlinkat` boot — slow but it gets there in ~9 min): the rejection is 100% internal to `libroblox` on CORRECT bytes — NOTHING Eclipse-side to fix.** Findings: the engine NEVER `openat`s the FS-extracted pack (`files/assets/shaders/…pack`) — it reads the pack from the REAL merged APK via its OWN zip reader (the merged `.apk` is `openat`'d **2435×** — the "pathological zip re-scan"), through **host glibc** (Eclipse does NOT mediate the read: no `AAssetManager` offset, no asset-serving). There are ZERO failed `openat`s anywhere; at the render stage the engine is reading `/sys/.../cpufreq/scaling_cur_freq`, not failing on a file. So "Error opening shader pack" is NOT an open/IO failure and NOT an Eclipse mis-served-asset bug — the engine reads byte-perfect APK bytes (verified `sha256`) and rejects them in its own parse, for the (gated) reason above. ⇒ Every Eclipse-side hypothesis (asset serving, FS extraction, offsets, file access) is now eliminated by direct syscall observation; the only remaining variable is libroblox-internal + server-flag-gated.
  SEPARATE Eclipse env issue noted (NOT the shader cause — it occurs ~5 s AFTER the reject): `RbxStorage` SQLite content-cache fails to open (`SqliteCache: Failed to open database … rbx-storage.db`, engine removes+retries). Worth a future look (content caching) but independent of the shader-pack parse.
- **2026-06-14 — 🔑 SHADER-PACK REJECT: APK-SIGNATURE/TAMPERING ANGLE — INVESTIGATED, LIKELY RED HERRING (could not confirm first-party).** Only one untried test remains (device-pulled production-signed splits, below); expect negative → the gated RBXS-container check is still the true boundary.
  After eliminating every *input the engine reads* (device, memory, GL renderer, surface extent [Phase 7], version), the remaining class was "integrity/anti-tamper." NEW EVIDENCE this session pins it to **provisioning, not an opaque RBXS check**:
  * **Every Roblox APK on disk is bundletool-extracted and NOT Roblox-production-signed.** `roblox-2.721.1108.apk` (the original used through the whole saga), `com.roblox.client.apk`, `base.apk`, `roblox-2.724.735-merged.apk` → **`v1sig=NONE`, no v2/v3 APK Sig Block**. The arch splits (`split_config.x86_64.apk`, `config.arm64_v8a.apk`) → `META-INF/BNDLTOOL.RSA` = **bundletool's debug key**. So the production signature was stripped when these were extracted from the `.aab` via bundletool. ⇒ **the signature/integrity hypothesis was NEVER actually tested — every APK tested was tampered.**
  * Eclipse's boot log shows `Package com.roblox.client has no certificates ... ignoring!` (ATL `PackageParser.collectCertificates`, V1/JarFile path) AND `StandardIntegrity : warmUpIntegrityToken` (Roblox runs Play Integrity). But ATL **ignores** the missing cert and the unsigned boot still reaches the render/shader stage — so ATL's "no certificates" is NOT itself the blocker; any signature check that bites the shader pack is **native (in libroblox), not ATL's Java package parse.**
  * Reference impl **Sober** (vinegarhq) *verifies the APK signature* and tells users to source an **untampered** APK from APKMirror (which preserves Roblox's Play signature). bundletool emits unsigned APKs (`INSTALL_PARSE_FAILED_NO_CERTIFICATES`). So Sober runs a production-signed package; Eclipse runs a tampered one — the concrete difference.
  * ⚠️ **INVESTIGATED 2026-06-14 — could NOT confirm first-party; now looks more like a RED HERRING than the cause.** Three things weakened it:
    1. **The strong counter-evidence:** people sideload these SAME bundletool/debug-signed Roblox APKs onto real Android and they render — so a *production* signature cannot be the engine's gate. (The one narrow survivor: a sideloaded app is *installed* so its V1 cert is registered, whereas Eclipse's MERGED APK uniquely has even its V1 sig stripped — but see below.)
    2. **Re-signing to test is fully confounded (proven twice):** both vendored-JDK21 `jarsigner` (V1) AND `uber-apk-signer` (clean V1+V2+V3, zipaligned, `signature verified [v2,v3]`) → ATL STILL logs `no certificates` (its JarFile parser doesn't pick up a re-applied sig) AND both **crash the boot identically** (`JNI error driving the framework lifecycle: Java exception` → `libroblox` SIGSEGV at the same addr `+0x23e74d0`) BEFORE the shader stage. Adding V1 META-INF entries / re-zipping breaks Eclipse's ATL asset path regardless of signature. So re-signing the local APK can NEVER test this.
    3. **Sourcing a pristine production-signed x86_64 APK FAILED (user authorized the attempt):** APKMirror (the only signature-preserving mirror) is Cloudflare-JS-locked (403, curl/WebFetch can't pass); APKPure's direct API (`d.apkpure.com/b/XAPK/com.roblox.client`) serves a **bundletool-tampered ARM (`armeabi_v7a`) XAPK** — wrong arch AND unsigned/debug, same as what we have; the IA splits are `BNDLTOOL.RSA`. **No automatable source yields a Roblox-production-signed x86_64 package** (x86_64 is Play-App-Bundle-only; every mirror that extracts via bundletool strips the prod sig).
  * **ONLY remaining clean test (needs the user + a device):** pull the Roblox-signed splits from an Android device/emulator that has Roblox installed via Play (`pm path com.roblox.client` → copy `base.apk` + `split_config.x86_64.apk`, which keep Roblox's real Play signature, NEVER re-zip/merge them) and boot Eclipse against the base for assets. This is the one untried way to settle the signature angle — but given counter-evidence #1, expect it to come back negative and return to the gated RBXS-container-check boundary. Do NOT burn more effort re-signing local APKs (confounded) or scraping mirrors (all tampered).
  * SEPARATE LATENT BUG surfaced: Eclipse deterministically crashes (`libroblox+0x23e74d0`) on a **zipaligned / extra-META-INF** APK — but REAL Play APKs are zipaligned, so this would bite a genuine production APK too and is worth a look before the device-pull test.
- **2026-06-14 — ✅ FS/EXTRACTION/PATH CLASS FULLY ELIMINATED (the pack the engine reads is provably perfect).** The engine opens the pack as a direct FS file `…/files/assets/content/../shaders/shaders_vulkan_mobile.pack` (Eclipse-extracted, as REAL Roblox also extracts content to `files/`). Verified ALL of:
  * **Fresh, not stale:** current FS copies = 18056115 / 9138476 bytes, mtime today, **exactly match the 2.724.735 APK** (the 20226723/9592046 in old `strace-full.log` were a stale 2.721.1108 boot — disregard that strace).
  * **Byte-perfect extraction:** FS copy `sha256` == APK entry `sha256` for both packs (APK entries are STORED). No corruption.
  * **Openable:** `files/assets/content/` EXISTS, so the `content/..` traversal resolves and the file opens fine — "Error opening shader pack" is therefore a **PARSE rejection mislabeled "opening,"** not an ENOENT/IO failure.
  * **Valid container:** both start `RBXS 0b 00 01 00` (magic + version 0x0b), identical APK↔FS.
  ⇒ The pack content, version, extraction, and file access are ALL perfect. The reject is unambiguously **internal to libroblox's RBXS parse and environment-sensitive** (real Android accepts the identical bytes). Every first-party-observable input is now eliminated; the specific reason is gated behind an `FLog` level Roblox's allowlist blocks, readable only by RE'ing `libroblox` (off-policy). The lone untried lever remains the device-pulled production-signed splits (above), expected negative.
  * Also ruled out 2026-06-14: **decompression** — the engine's zstd works (boot log: `writeFlagCache: Compression complete … ratio 3.50x`; `libzstd-jni` loads, 144 natives bound), so the RBXS directory isn't failing a zstd path. Noted-but-independent: two unbound View natives fire at render-init (`View.nativeIsAttachedToWindow`, `View.getWindowVisibleDisplayFrame` → `UnsatisfiedLinkError`) — but the engine logs the shader-pack reject (not these) as the `RenderView is NULL` cause, and `getWindowVisibleDisplayFrame` fires AFTER the reject. Binding them is good framework hygiene (next-step discovery signals) but is NOT the render fix.
  * **strace attempt (2026-06-14) — INCONCLUSIVE for the open, but two facts:** (1) a `-e trace=%file` strace of the boot captured the engine `statx`-ing both packs with **`= 0` success and correct sizes** (18056115 / 9138476) → the file is found; reinforces "parse rejection, not open failure." (2) the traced boot **crashed before render-init** (strace uses `ptrace`). ⚠️ Do NOT over-read this as proven Hyperion anti-debug: crash offsets are NOT consistent (`+0x23e74d0` on the re-signed-APK boots vs `+0x2779bb0` elsewhere), so "single anti-tamper handler" is UNSUPPORTED. The honest read: the engine is fragile to environment perturbation (ptrace / re-zip) — *could* be anti-tamper, *could* be an Eclipse loader/TLS/relocation fragility — unconfirmable without RE. Net: the shader-pack OPEN is not observable via tracing (the trace perturbs the engine into an earlier crash), so the parse-internal reject reason stays gated behind the off-policy boundary.
- **2026-06-14 — 🖼️ RENDER PHASE 7 SHIPPED: WAYLAND UNDEFINED SURFACE-EXTENT FIX.**
...
- **2026-06-13 — 🖼️ RENDER PHASE 6 SHIPPED: VULKAN WSI TRANSLATION (Android→Wayland) via a tier-0 `dlsym` interposer.
  ...
  **FFlag OBSERVABILITY PATH PURSUED + BLOCKED (2026-06-13):** the legitimate way to surface the engine's OWN reject
  ...
  NEW LEAD (2026-06-13, unfiltered log scan, for the NEXT attempt): the host GLES context the engine sees exposes a DESKTOP-NVIDIA extension set no real Android GPU has (`GL_NV_draw_vulkan_image`/`path_rendering`/`bindless_texture`/`mesh_shader`/…) + `Shader program binary support is: enabled`. The engine's variant-selection (synthesis hypothesis #1, `shader pack does not have variant %s`) likely fingerprints the GPU/feature-set; Eclipse's desktop-NVIDIA-as-GLES fingerprint may match NO variant in the mobile-targeted pack. Testing it = spoofing `glGetString(GL_RENDERER/EXTENSIONS)` + caps to look like a real mobile GPU (Adreno/Mali) — INVASIVE (intercept many GL queries) + still guessing the variant-key without confirming the mechanism (RE), GL-FINGERPRINT LEAD TESTED + DISPROVEN (2026-06-13): added a tier-0 `glGetString` spoof (registered in `native_provider` like `eglGetDisplay`; gl* ARE UND imports, the tier-0 intercept WORKED — engine logged `GL Renderer: Adreno (TM) 640` / `GL Vendor: Qualcomm` / `Excluded 'HTC unknown:Adreno (TM) 640'`) — but the GLES shader pack STILL rejected. DECISIVE: the reject is API-AGNOSTIC — Vulkan (Mode 6) never calls `glGetString` yet fails at the SAME early-parse step, so the common cause is the RBXS CONTAINER parse itself, NOT the GL renderer/vendor/device fingerprint. Reverted the diagnostic (clean Phase-6 tree). This rules out the last concrete environment-fingerprint hypothesis; the reject is an in-engine RBXS container check (version/format/integrity) reachable only by RE (off-policy) or the allowlist-blocked FFlag logging. Detail: §6 (2026-06-13 render Phase 6).
...
### 2026-06-14 — ✅✅ SHADER-PACK REJECT ROOT-CAUSED + FIXED: `AAsset_openFileDescriptor` was a `-1` stub; serve a real `memfd`-backed fd → the engine LOADS the shader pack, creates `RenderView`, reaches the LuaApp landing screen (commit `ac5feba`)

  *Root cause (first-party, NOT internal/gated/off-policy as every prior 2026-06-14 entry concluded):* `eclipse_aasset_openfiledescriptor` (`src/loader/native_provider.rs`, the NDK `AAsset_openFileDescriptor`) returned `-1` unconditionally, on the assumption that an in-memory asset has no backing fd so callers fall back to `AAsset_getBuffer`. libroblox's shader-pack loader does NOT fall back: for the large **STORED** packs (`vulkan_mobile` ~18 MB, `glsles3` ~9 MB) it uses the fd/`mmap` path and treats `< 0` as fatal → `Error opening shader pack <name>` → `RenderView is NULL` → no frames. This single stub was the COMMON blocker behind every Mode 6 (Vulkan) and Mode 4 (GLES3) failure for the whole render investigation.

  *Fix:* Eclipse already holds each asset's full decompressed bytes (`AssetState.bytes`, byte-exact with the APK entry), so back them with an anonymous in-memory file — `memfd_create` → `ftruncate` → write the bytes → rewind → return its fd + `[0, len)` (the real-Android contract: an fd the caller can `mmap`, not a fake fd). Falls back to the documented `-1` only on a stale handle or a checked syscall failure (caller then uses the buffer path). All `unsafe` carries a SAFETY note; every libc call is checked.

  *This corrects the prior strace conclusion* that "Eclipse does NOT mediate the read — no `AAssetManager`, no asset-serving." That was FALSE: the 2435× merged-APK `openat`s observed under strace were Eclipse's OWN `eclipse_aassetmanager_open` re-opening the APK per asset (`crate::apk::Apk::open` opens it each call); the shader pack went through `AAsset_openFileDescriptor` → `-1` → reject. The bytes were byte-perfect (that finding held) but the fd PATH was the bug — "correct bytes ⇒ Eclipse mediates nothing" was a non-sequitur.

  *Regression protection:* the `aasset_openfiledescriptor` unit test is rewritten from "asserts `-1`" to require a REAL fd with byte-exact contents (`fd >= 0`, `out_start == 0`, `out_length == len`, full read-back == source) — fails on a regression to the `-1` stub OR any wrong-bytes/offset bug. Tied directly to the confirmed root cause; no new script.

  *Verification:* `cargo fmt`/`build --all-targets`/`clippy --all-targets --all-features -D warnings` 0-warn / `cargo test` **567 unit + integ + 2 doctest, 0 failed** / `cargo build --release` clean. OWNER LIVE BOOT (`/tmp/boot_fd.log`, patched framework `ECLIPSE_ANDROID_FRAMEWORK_DIR=$HOME/.cache/eclipse/framework-patched`, EXIT=124 clean 240 s / 1311 lines): `Loaded 3470 shaders from pack vulkan_mobile variant default (17967131 bytes)` · `RenderView created[1]` (was NULL) · `Vulkan: swapchain images 3 … size 800x600` · `SurfaceController::RenderJob` running · `onDataModelNotification() type:APP_READY data:Landing` · `LuaAppStarterScript` running. Remaining errors are only HTTP 403 on `user-heartbeats-api/pulse` — expected (no authenticated user session), not a render failure.

  *Next:* confirm pixels actually present to the winit window (fresh boot + screenshot); then the LuaApp landing/login UI render layer (auth-gated heartbeats are separate). Audit other `-1`/stub NDK natives a fall-back-less caller could treat as fatal.

  *Files:* `src/loader/native_provider.rs` (`eclipse_aasset_openfiledescriptor` real-fd impl + rewritten regression test), `AGENTS.md`.

---

### 2026-06-14 — ✅ HOST INPUT FORWARDED: bounded main-Looper pump (fixes the event-loop freeze) + engine-mode pointer path via `onTouchEventInternal` + `ValueCallback` interface — clicks reach Roblox (Landing → LoginV2)

  *Root causes (all [confirmed] by live boot):* (1) `pump_main_looper` called `android.os.Looper.loop()` once per `about_to_wait`; `loop()` only returns when the queue drains empty, but the landing screen's auth/fragment error-retry paths re-post main-thread messages continuously → `loop()` never returns → the winit event loop FROZE → no host input delivered (main thread spins `RNl`). (2) The engine's input route is ATL's `View.onTouchEventInternal(MotionEvent, boolean)` (View.java:1133), which invokes the view's `OnTouchListener` — NOT `dispatchTouchEvent` or `onTouchEvent`, both `{ return false; }` stubs in ATL (View.java:2062/1147). (3) The post-handoff pointer path was dead (`handle_primary_press`/`release` early-return on `self.renderer == None`, dropped at handoff) and targeted Eclipse's own view tree. (4) `android.webkit.ValueCallback` shipped as a class not an AOSP interface → `IncompatibleClassChangeError` in `CookieProtocol` (one storm source).

  *Fixes (one commit):* (a) `drive_main_messages` (`src/framework.rs`) replaces the single `Looper.loop()` with a BOUNDED batch (`MAIN_LOOPER_MESSAGE_BUDGET = 512`) mirroring ATL's loop body (`myQueue → next → getTarget → dispatchMessage → recycle`), each message in its own local frame, per-message throw described+cleared. Idle drains identically to `loop()`; a storm caps per tick so `about_to_wait` stays responsive. (b) `dispatch_touch_to_engine_surface`/`touch_engine_surface` + `engine_surface_view_handle` (`src/framework.rs`) build a `MotionEvent` (constant downTime across DOWN/UP) and call `onTouchEventInternal(event, false)` on the `RBXSurfaceView`. (c) `engine_primary_press`/`engine_primary_release` + `handed_off` branch + per-event `feed_winit_input_to_loopers` wake + env-gated `ECLIPSE_SYNTHETIC_ENGINE_TAP` diagnostic (`src/graphics.rs`). (d) `android/webkit/ValueCallback.java` interface in the overlay (`tools/framework-overlay/`, classes.dex first-dex-wins) + a `patch-framework.sh` grep guard.

  *Verification:* gate green (fmt/clippy/567 unit + 4 integ + 2 doctest/release). OWNER LIVE BOOT (`/tmp/boot_oti.log`, patched framework): synthetic tap at the Sign In button (400,390) → `APP_READY data:LoginV2` + `ActivityNativeMain onAppReady: LoginV2` (Landing→Login navigation); the owner's subsequent window clicks all log `consumed=true`.

  *Regression protection:* the `ValueCallback` overlay grep guard in `patch-framework.sh` (fails the build if it regresses to a class); the bounded pump + JNI dispatch are validated by the dev-host live boot (ART can't run under `cargo test`, per `docs/dev-host-runbook.md`) — the existing 567+4+2 gate still passes (no regression to the lifecycle the pump drives).

  *Known follow-ups (do NOT block input):* `RBXSurfaceView` is never laid out (`getWidth/Height == 0`; harmless for the listener path); ACTION_MOVE/hover + scroll + keyboard forwarding; login NETWORK gaps (`CookieManager.setCookie(String,String,ValueCallback)`, `JobParameters.getNetwork()`, `View.nativeIsAttachedToWindow`/`getWindowVisibleDisplayFrame` + the 403s); SIGTERM/`/tmp` cleanup handler.

  *Files:* `src/framework.rs`, `src/graphics.rs`, `tools/framework-overlay/src/android/webkit/ValueCallback.java` (NEW), `tools/framework-overlay/patch-framework.sh`, `AGENTS.md`. Input investigation: workflow `eclipse-input-forwarding-investigation`.

---

### 2026-06-14 — ✅ POINTER DRAG + KEYBOARD forwarding (extends the input subsystem): ACTION_MOVE while held; KeyEvent via dispatchKeyEvent with a populated unicodeValue

  *Drag:* added `MotionAction::Move` (`ACTION_MOVE = 2`) + `graphics.rs` `engine_pointer_move`, dispatched on `CursorMoved` while the primary button is held (reusing the press downTime) so the engine tracks drags (scroll lists, sliders).

  *Keyboard:* `WindowEvent::KeyboardInput` (engine mode) → `graphics.rs` `engine_key` → `framework::dispatch_key_to_engine_surface`. Builds an `android.view.KeyEvent` (6-arg ctor `(JJIIII)V`), JNI-`SetIntField`s the package-private `unicodeValue` (ATL's `getUnicodeChar()` returns it verbatim — its keymap path is disabled — and the public ctors leave it 0, so without this nothing types; this replicates what ATL's GTK key driver does), and calls `RBXSurfaceView.dispatchKeyEvent(KeyEvent)`.

  *Entry-point resolution (live boot, confirm-then-build):* tried `onKeyDown`/`onKeyUp` first per one facet; the boot proved `onKeyUp` is ABSENT on the view (`GetMethodID … returning NULL`) and base `onKeyDown` is a `return false` stub — so the engine overrides `dispatchKeyEvent` (the synthesis's pick, the route ATL's own GTK driver uses). Switched to `dispatchKeyEvent`: both DOWN+UP now dispatch with NO error (`consumed=false` only because the synthetic tap hit a button, not a focused field). `winit_keycode` maps A–Z/0–9/space/./@/-/+/slash/backspace/enter/tab/esc/arrows/home/end; modifiers maintain a running `META_*` bitmask; the typed char rides on `unicodeValue` so even an unmapped printable types.

  *Privacy:* keystrokes are NEVER logged (only `consumed` + press/release half) — credentials must not leak to the log.

  *Verification:* gate green (fmt/clippy/test + release). New pure regression tests: `key_action_codes_match_public_android_constants`, `key_event_dispatch_descriptors_are_the_public_android_api` (incl. the `unicodeValue` field name+`I`), `winit_keycode_maps_credential_keys_to_android_keycodes`. OWNER LIVE TYPING TEST PENDING (click a LoginV2 text field → type → confirm the char appears + `consumed=true`). Fallback if it doesn't type: route text through the focused `EditText`'s `TextWatcher` (AGENTS §5 note).

  *Files:* `src/framework.rs` (KeyAction, KEY_EVENT_CLASS, dispatch_key_to_engine_surface/key_engine_surface, MotionAction::Move, tests), `src/graphics.rs` (engine_pointer_move, engine_key, winit_keycode, KeyboardInput arm, key_meta_state, ECLIPSE_SYNTHETIC_KEY, test), `AGENTS.md`. Keyboard investigation: workflow `eclipse-keyboard-input-investigation`.

---

### 2026-06-14 — 🔬 Engine text-bridge surface MAPPED + fully exercised; typed text FORWARDED via the engine's own protocol, but VISIBLE login text is an architectural (Android-View compositing) gap, NOT a missing bridge

  *What/why:* The goal is VISIBLE login typing. Prior work made typing FUNCTIONAL (EditText `TextWatcher` → engine validates → "Next" enables) but the character never drew in the field box. To find the engine's real text path WITHOUT RE'ing `libroblox` (cyber-safeguard), added `framework::reflect_engine_input_methods` (env `ECLIPSE_REFLECT_INPUT`, fired ~8s post-handoff from `graphics`) — a JNI reflection dump of the `com.roblox.engine.jni.NativeGLInterface` / `NativeInputInterface` declared methods. It revealed the COMPLETE text-bridge set: `nativePassText(long,String,boolean,int)`, `syncTextboxTextAndCursorPosition2(String,int)`, `updateKeyboardSize(boolean,int,int,int,int)`, `nativePassKeyEvent(boolean,int,int,boolean)`, `nativeReturnPressedFromOnScreenKeyboard(long)`, `nativeGetTextBoxInfo()→NativeTextBoxInfo`.

  *Done:* `type_into_active_text_field` now also forwards each keystroke through the engine's OWN protocol (alongside the EditText TextWatcher): `sync_engine_textbox` → `updateKeyboardSize(true,…)` (signal keyboard-shown) + `nativePassText(0,text,false,cursor)` (IME commit) + `syncTextboxTextAndCursorPosition2(text,cursor)`; `pass_key_event_to_engine` → `nativePassKeyEvent` DOWN+UP per char (`android_keycode_for` maps codepoint→`KEYCODE_*`+metaState, unit-tested).

  *Confirmed mechanism (exhaustive elimination, live boots):* every bridge delivers cleanly (0 throws, no crash, `handled=true`) yet the field renders empty and the presented frame is **byte-identical** (`grim`, 17708 B) across `nativePassText` / `syncTextbox` / `updateKeyboardSize` / `nativePassKeyEvent` boots — NO engine text/input bridge visually renders typed text on Eclipse. A one-shot reflection of the live `NativeTextBoxInfo` (diagnostic since removed) showed it carries ONLY geometry/style (`x,y,width,height,font,fontSize,textColor,textInputType,multiline,returnKeyType,alignment`) — **no `text`, no `cursor`, no handle** field, a fresh instance per call. ⇒ (a) `nativePassText`'s `long` is an eventTime, not a textbox pointer (Java is handed no handle), so `0` is crash-safe (boot-confirmed: process alive); (b) the visible login text is a native Android `EditText` the OS composites over the GL surface (positioned via `NativeTextBoxInfo`), which Eclipse's GL-surface-only post-handoff presentation does NOT composite → invisible even though the engine receives + validates it. Pinning the engine's exact internal render trigger would require RE of `libroblox` (cyber-safeguard OFF-LIMITS).

  *Conclusion:* text input FORWARDING is complete (the engine receives typed text via its real protocol). VISIBLE typing needs an architectural phase — an Android-View overlay compositor (draw the focused `EditText`'s text over the engine surface) or engine-IME-composition emulation (RE-blocked) — NOT another bridge call. Per CLAUDE.md, stopped guessing bridges once all were exhausted and the mechanism was confirmed by evidence.

  *Verification:* gate green (fmt / clippy `-D warnings` / 572 unit + 4 integ + 2 doctest / release). New pure regression guard `android_keycode_for_maps_credential_keys` (KEYCODE_A=29…Z=54, 0=7…9=16, space=62, DEL=67, META_SHIFT_ON for uppercase, unmapped→None). OWNER live boots (`/tmp/boot_keyevent.log`, `boot_kbsize.log`, `boot_passtext.log`): synthetic tap→focus→type "robloxtest" → `handled=true`×10, 0 throws, process alive; `/tmp/keyevent_field.png` ≡ `kbsize_field.png` (byte-identical empty field).

  *Files:* `src/framework.rs` (`reflect_engine_input_methods`, `sync_engine_textbox`, `pass_key_event_to_engine`, `android_keycode_for` + test, `type_into_active_text_field` wiring), `src/graphics.rs` (`engine_reflect_done` + the env-gated reflect call), `AGENTS.md`.

---

### 2026-06-14 — 🖼️ VISIBLE-TEXT OVERLAY Step A: Vulkan present-path interception + engine-object capture (the seam for a MangoHud-style text overlay), LIVE-VALIDATED

  *Why:* visible login typing needs Eclipse to draw OVER the engine's frame. The engine renders + presents via **Vulkan** (live boot: NVIDIA RTX 5070, `swapchain images 3 … size 800x600`, `vkQueuePresentKHR`), and post-handoff the engine owns the single `wl_surface` (Eclipse dropped its renderer; Wayland can't subsurface across winit's connection) — so the engine's per-frame Vulkan present is the ONLY compositing seam. The overlay is therefore a present-path injection, mirroring Eclipse's existing tier-0 Vulkan-WSI interposition.

  *Done (Step A):* new module `src/loader/vk_overlay.rs`. `vulkan_wsi::eclipse_vk_get_instance_proc_addr` now also returns Eclipse's `vkCreateDevice` + `vkGetDeviceProcAddr` shims (via `vk_overlay::intercept_instance_proc`); the gdpa shim returns `vkQueuePresentKHR`/`vkCreateSwapchainKHR`/`vkGetSwapchainImagesKHR` shims. Each caches the host command (the `HOST_PDSC` atomic pattern) and forwards ABI-exact. The capture shims record the engine's `VkDevice`, swapchain handle + `image_format` (37 = `VK_FORMAT_R8G8B8A8_UNORM`) + `image_extent` (800×600) + the 3 swapchain `VkImage`s (all handles stored as raw `u64` so the `Mutex<OverlayState>` static is `Send`). `vkQueuePresentKHR` is a pass-through that logs ONCE that the seam is armed (`device_set/swapchain_set/images/image_index` all populated), then forwards to the host present.

  *Verification:* gate green (fmt / clippy `-D warnings` / 578 tests / release). OWNER LIVE BOOT (`/tmp/boot_vkpresent.log`): `captured engine VkDevice` + `captured engine swapchain format=37 800x600` + `captured swapchain images count=3`, then `eclipse_vk_queue_present_khr interposed … device_set=true swapchain_set=true images=3 present_swapchains=1 image_index=0` firing at the engine's frame rate, process alive throughout (no crash/corruption) — the injection seam is proven and every object the draw needs is in hand. No unit test (pure FFI interposition, like the rest of `vulkan_wsi`; validated by live boot per `docs/dev-host-runbook.md`).

  *Next (Step B):* build the overlay's Vulkan resources on the captured device (render pass for `R8G8B8A8_UNORM`, pipeline, vertex buffer, glyph texture via the existing `ab_glyph`/`tiny-skia` text path) and record a synchronized command buffer in `vkQueuePresentKHR` that draws the focused field's text into `images[imageIndex]` before the host present — solid quad first (prove compositing), then text, then live field-tracking + caret. Performance: the present hook only does work when a text field is focused (login); a pure pass-through otherwise (zero gameplay overhead).

  *Files:* `src/loader/vk_overlay.rs` (NEW), `src/loader.rs` (mod), `src/loader/vulkan_wsi.rs` (gipa wiring), `AGENTS.md`.

---

### 2026-06-14 — 🖼️ VISIBLE LOGIN TEXT, Step B: native-render path RULED OUT (fights the engine); Vulkan present-path text OVERLAY renders the focused field's text — VERIFIED by a self-contained field probe

  **Native verdict (owner asked to chase native first).** Delivering each keystroke to the engine as IME COMPOSING text (`nativePassText` `isComposing=true`, env `ECLIPSE_TEXT_COMPOSING`) DID make the engine draw the field text live (owner: "i see it while typing") — so the engine *can* render live. BUT it is intractably buggy in-policy: one keypress renders 3× (multiple delivery paths each enter the char — `text_paths()` env added to isolate them: `w` TextWatcher, `p` nativePassText, `s` syncTextbox, `k` nativePassKeyEvent, `e` updateKeyboardSize) and the field auto-defocuses per keystroke (the engine drops the edit session without the right keyboard-session signalling). Matching the engine's exact text-input state machine needs RE of `libroblox` (cyber-safeguard OFF-LIMITS). ⇒ forcing the engine to live-render fights its design (on Android the LIVE text is the platform's native `EditText`, composited by the OS — never the engine, which only commits on defocus). The **overlay is therefore the platform-correct fix**, not a workaround: Eclipse does the Android input-View compositing the platform owes the app.

  **Overlay (Step B, landed, env-gated `ECLIPSE_VK_OVERLAY`).** `vk_overlay`'s present hook now does READ-MODIFY-WRITE on the engine's swapchain image: copy the focused field's rect out → CPU-rasterize the field's text onto those pixels (`ab_glyph` + `graphics::discover_font_path`, white glyphs alpha-blended, preserving the engine's bg/border) → copy back, then present with the engine wait-semaphores cleared (the readback consumed them). Drawn EVERY frame when a field has text (else the engine repaints over it → flicker); zero work when no field is focused (gameplay untouched). Text source: `framework::active_text_field_text()` (the focused EditText's registry text), or env `ECLIPSE_VK_TEXT_TEST` for focus-free render tests.

  **Self-contained verification (the dev host CANNOT screenshot the buried window — Opera fullscreen dominates the compositor).** A field PROBE (env `ECLIPSE_VK_PROBE`) reads the field rect back and logs an ASCII ink-profile sparkline. With `ECLIPSE_VK_TEXT_TEST=robloxtest` the probe logged `| ==*@=@#=*##=#.  …|  total_ink=425` — left-aligned glyph-shaped ink composited INTO the engine's frame, stable across frames, engine alive. The overlay text rendering WORKS, verified compositor-independently.

  **Verification:** gate green (fmt / clippy `-D warnings` / 578 tests / release). Default boot (no overlay/probe env) reaches LoginV2, overlay+probe off, alive — no regression. Dead B1 solid-quad `OverlayRenderer` (the compositing proof) removed once the read-modify-write path superseded it.

  **REMAINING for the polished default:** (1) the login layout DRIFTS across boots (the fixed `login_field_rect` 181,149,438,46 was stale — the field moved), so the overlay must track the field's LIVE geometry (`NativeTextBoxInfo` x/y/w/h — needs a safe JNI source, not the registry which stores only requested layout params); (2) reliable focus for autonomous testing (the engine's focus-on-tap is flaky; the synthetic now retries until `framework::active_text_field()` is set, but the drifted layout means the tap coord is stale); (3) match the field's `fontSize`/`textColor`; (4) make the overlay the default (drop the env gate) once 1–3 land. The hard part (compositing text into the engine's Vulkan frame) is DONE.

  *Files:* `src/loader/vk_overlay.rs` (read-modify-write overlay + probe + `ab_glyph` text raster), `src/loader/vulkan_wsi.rs` (VkInstance capture), `src/framework.rs` (`active_text_field`/`active_text_field_text`, `text_paths`/composing experiment gates), `src/graphics.rs` (`discover_font_path` pub(crate), synthetic focus-retry), `AGENTS.md`.

---

### 2026-06-14 — ✅✅ VISIBLE LOGIN TYPING WORKS (DEFAULT-ON): Vulkan text overlay (display) + TextWatcher (detection) — username typed, shown, RESOLVED to the real account, ADVANCED to the password step

  **Outcome.** The user can type in Roblox's login field and SEE it, and the engine DETECTS it: a synthetic run (tap "Sign in to another account" → LoginV2 → focus the username field → type "robloxtest" → tap Next) RESOLVED the username to the real account **RobloxTest/@robloxtest** and ADVANCED to the password screen — all verified by autonomous PNG screenshots. Default behavior, no env flags.

  **The two halves (this is exactly how Sober does it, Eclipse-style — Sober uses GTK to composite the field + the engine bridges to deliver; Eclipse can't use GTK, so the Vulkan overlay is the display half):**
  - **DISPLAY = the Vulkan present-path overlay** (`vk_overlay`, ON by default; opt-out `ECLIPSE_NO_VK_OVERLAY`). Each present where a field is focused, it READ-MODIFY-WRITEs the swapchain image: copy the field rect out → CPU-rasterize the field's text onto those pixels (`ab_glyph`, white over the engine's bg/border) → copy back, present with the engine wait-semaphores cleared. A cheap atomic fast-path skips ALL work when no field is focused → zero gameplay cost. The field rect is the engine's LIVE geometry: `framework::query_textbox_geometry` reads `NativeTextBoxInfo` x/y/w/h on the JNI-attached main thread (the login layout DRIFTS — a fixed rect goes stale) → cached for the present thread.
  - **DETECTION = `text_paths` default `w`** (the EditText `TextWatcher` → `RbxKeyboard` reads `getText` + delivers to the engine). This is the ONLY engine bridge fired; the composing/`nativePassText`/`syncTextbox`/`nativePassKeyEvent` paths were RULED OUT — composing made the engine draw live but triples each char (multiple delivery paths) AND auto-defocuses per keystroke (fights the engine's edit-session protocol; matching it needs off-limits `libroblox` RE). `w` validates WITHOUT either bug (`handled=true` ×10, no defocus).

  **Verification tooling (the dev host CANNOT `grim` the buried Eclipse window — Opera's fullscreen dominates the compositor):** a self-contained, dependency-free PNG encoder (`encode_png_rgba` — uncompressed DEFLATE + CRC32/Adler32) lets the field probe (`ECLIPSE_VK_PROBE`) write `/tmp/eclipse_field_probe.png`; `ECLIPSE_VK_SCREENSHOT` makes it capture the FULL frame. This gave fully-autonomous screenshots: I could see the account-picker (which is why the old `400,390` "Sign In" tap missed — the button is "Sign in to another account" at `400,466`), navigate, and confirm each step. The synthetic harness gained focus-retry (re-tap until `active_text_field()` is set) + a stage-3 Next-tap.

  **Verification:** gate green (fmt / clippy `-D warnings` / 578 tests / release). No-env default boot reaches LoginV2, overlay armed, no crash/Vulkan-error, alive. Autonomous PNG proofs: field shows "robloxtest"; Next → password screen with RobloxTest resolved.

  **Known follow-ups (do NOT block the win):** the per-present readback+writeback slows the framerate while a field is focused (fine for login; optimize to redraw-on-text-change / a GPU pipeline later); caret/blink; the password field (same flow); the LAN-IP/network login completion.

  *Files:* `src/loader/vk_overlay.rs` (default-on overlay, live-geometry `field_rect`, present fast-path, `encode_png_rgba` + full-frame screenshot, probe), `src/framework.rs` (`text_paths` default `w`, `query_textbox_geometry`/`textbox_geometry`), `src/graphics.rs` (synthetic focus-retry + Next-tap stage, geometry query call), `AGENTS.md`.
...
### 2026-06-14 — 🔒 Login FORM complete: password fields MASKED (•, never plaintext) + framerate verified 60fps — full username→password flow works
  ...
  **Verification:** gate green (fmt / clippy `-D warnings` / 578 tests / release). Autonomous PNG proofs: username "robloxtest" visible; Next → password screen (RobloxTest); password "secret123" → `••••••••`. **Remaining (non-blocking):** actual login completion (real password + the LAN-IP/network path); audio. (Added since: a blinking caret `robloxtest|`; centered masked-password dots; mouse-wheel scroll forwarded to `nativePassMouseWheel` — harmless by construction, but its visual scroll effect is unverified without a scrollable surface / real-mouse user.)
...
### 2026-07-10 — 🌉 WEB-ENGINE MILESTONE M4 DONE: JS bridge + cookies + honest UA via the additive protocol-v2 extension — async-Promise bridge (renderer V8 + CefMessageRouter → ART reflect-invoke), session-scoped private cookie store, honest deliberate UA in BOTH units, the javascript:-println leak channel patched, and the post-review threading hardening (the upcall thread owns ALL app-facing JNI; era-gated exactly-once callback drains); gate green 629+6+2 root zero-SKIP / 23+15 helper; owner `__webview-test` EXIT=0; the live pointer advances to MILESTONE M5

*Provenance (the three-workflow chain):* implements MILESTONE M4 of `docs/web-engine-plan.md` (after the 🔗 M3
record above). (1) **wf_881986ec** — recon ×3 (helper crate / main-process seams / overlay+Java surface) →
design → implement → INDEPENDENT gate re-run in a fresh shell → adversarial review ×3 dimensions with
per-finding skeptic verification (7 alleged / 7 CONFIRMED / 0 refuted) → fix pass applying all 7 + re-gate.
(2) **wf_d4c555fc** — focused post-fix adversarial re-review of the fix code (2 dimensions): 4 alleged — 2
CONFIRMED, 1 REFUTED with mechanism, 1 left unadjudicated (its skeptic died on a session limit).
(3) **wf_ad0f573b** — close-out: the unadjudicated reader-JNI finding re-adjudicated (CONFIRMED, minor) + fixed,
post-fix findings A/B fixed (era-gated), stale protocol-v1 strings fixed tree-wide, full re-gate. Then the owner
live `__webview-test` run on the dev-host main thread per the dev-host boundary (live ART never under
`cargo test`/subagents). Scope held to the plan: the driven page is an OFFLINE first-party loopback page (real
`http://127.0.0.1:<port>` origin — no external network; the M3 live-roblox.com dependency moves to M6); no
libroblox preload, no challenge URL; "integrates, never duplicates" held (per-view identity stays the existing
`view_registry` widget handle).

*Load-bearing design decisions:*
- **`PROTO_VERSION = 2`, EXACT-MATCH negotiation — no capability bits.** Rationale: the helper is resolved from
  the SAME build tree every session, so consumer+helper always ship as one version; the existing exact-match
  handshake already gives the required loud/honest failure — a skewed peer answers `HelloAck{its version}` and
  closes, and the consumer latches the M3 one-shot-WARN no-op degradation (no fabricated callback, no crash).
  v1 layouts stay FROZEN: the v1 round-trip pin is untouched (still asserts exactly 24 messages) and the v2
  extension is purely ADDITIVE — new wire types ct `0x11–0x14` (`BridgeRegister`/`BridgeResult`/
  `EvaluateJsForResult`/`CookieSetForResult`) + ht `0x89–0x8B` (`BridgeCall`/`EvaluateJsResult`/
  `CookieSetResult`), all with caps enforced BEFORE allocation in the total-decoder shape (+ a separate v2
  round-trip/caps/totality pin set). Bridge/eval/cookie payloads are OPAQUE to the helper — forwarded unparsed
  and NEVER bound to a log macro (lengths/counts only); only ART parses/produces them.
- **Async-Promise bridge = the DOCUMENTED DIVERGENCE from AOSP's synchronous `@JavascriptInterface` contract.**
  CEF's only cross-process JS↔native primitive (`CefMessageRouter`/`cefQuery`) is asynchronous; a truly
  synchronous return would block the sandboxed renderer main thread on renderer→browser→socket→ART→back — CEF
  does not offer it and it risks renderer deadlock. Renderer-side V8 stubs make every
  `window.<name>.<method>()` return a Promise; the renderer main thread is NEVER blocked. A page expecting a
  synchronous return value receives a Promise instead — recorded, to be re-validated against `RBHybridWebView`'s
  real bridge shape at M6 (plan open question #3; the completion token flows page→native and needs no
  synchronous return). Security gate = the `@JavascriptInterface` annotation (RUNTIME-retained, a new overlay
  javac class) checked by reflection — only annotated methods cross; overloads are retained per name and
  resolved by ARGUMENT COUNT at call time per the Chromium gin java-bridge reference (name→candidates +
  `FindMethod(name, num_parameters)`; same-arity first-match deliberately mirrors gin). Inventory delivery is a
  PULL model — the renderer signals `eclipse.bridge.ready` on each main-frame context and the browser re-sends
  the per-view inventory (the design's push was live-observed dropped by CEF when sent before the renderer
  connects).
- **Honest deliberate UA.** `Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko)
  Chrome/149.0.0.0 Safari/537.36 Eclipse-WebView/149.0.6` — genuinely Chromium 149 on Linux x86_64, deliberately
  identifying via the `Eclipse-WebView` product token, never impersonating a device, no host detail beyond the
  canonical reduced desktop platform token. Set helper-side in `build_settings()` (no wire field, no argv — out
  of `/proc/cmdline`) AND returned by the overlay `WebSettings` (`getUserAgentString` + `getDefaultUserAgent`);
  byte-matched + pinned in both units; the recorded "GDPR VIOLATION" literal is GONE (grep-guarded). M6 owner
  caveat on vendor reception stays plan open question #1.
- **Session-scoped private cookie store.** ONE helper-process in-memory `RequestContext` (empty `cache_path`,
  `persist_session_cookies=0` — NOT the global store), created lazily, shared by EVERY browser (6th
  `browser_host_create_browser_sync` arg) and EVERY cookie op — so the CookieManager `.ROBLOSECURITY` handoff
  lands in the store the challenge WebView reads. The overlay `CookieManager` is native-backed end-to-end
  (get / 2-arg set / 3-arg set / removeAll / removeSession / flush-no-op — NEW registrar `bound=6`), replacing
  the recorded fabricated `Boolean.TRUE` with the real async result; `getCookie` is a bounded blocking
  round-trip (5 s, honest-empty degrade). WebView natives grew `bound=3` → **`bound=5`**
  (`native_evaluateJavascript` + `native_addJavascriptInterface`); the `javascript:`-println full-URL leak
  channel is overlay-patched (loadUrl routes `javascript:` URLs to `native_evaluateJavascript`, the println is
  gone) BEFORE such URLs become secret-bearing; WebView + WebSettings shadowed into classes2.dex and the two new
  javac classes (`JavascriptInterface`, the inert `EclipseBridgeProbe` test probe) into classes.dex, all behind
  the established anchor/pristine/back-check §-guards.
- **The dedicated `eclipse-webview-upcall` thread owns ALL app-facing JNI; the reader is fully JNI-free.** The
  root fix for the confirmed reader-reentrancy self-deadlock class: internalLoadChanged, bridge reflect-invokes
  (+ BridgeResult reply), every ValueCallback delivery, and the deferred closed-view bridge drop all run on the
  upcall thread (which owns the JavaVM, fed by an in-order mpsc channel); the socket reader keeps only
  staging/acks and the non-JNI getCookie-waiter channel wake (an `IO_THREAD_ID` boundary assertion fails fast if
  reader-side JNI is ever reintroduced). A synchronous `CookieManager.getCookie` from `onPageFinished`/an
  `@JavascriptInterface` method now parks the upcall thread while the free reader delivers the real CookieList —
  correct cookies, no stall.
- **Era-gated drains = exactly-once, honest-failure callback delivery on every loss path.** A monotonic
  `WEBVIEW_CLOSE_ERA` tags eval callbacks and BridgeEntries with their birth era; the reader bumps the era under
  the same views-lock hold that removes a ViewClosed view, and `ViewClosedDrain{upto_era}` fails only callbacks
  born BEFORE the close — closing the close+re-drive corner (a fresh post-re-drive callback survives a stale
  queued drain). `drain_all_webview_callbacks` fails every pending callback honestly exactly once (eval →
  `"null"`, cookie callbacks → `Boolean.FALSE`) on every reader exit (helper crash/EOF/protocol error, renderer
  death, deliberate shutdown); reader exit also wakes parked getCookie waiters immediately (Sender drop →
  Disconnected → honest empty); `notify_view_freed` releases bridge globals + pending inventories BEFORE the
  tracked-view gates (the never-driven-view leak).

*Review ledger:* **First review (wf_881986ec, 3 dimensions + skeptics): 7 alleged / 7 CONFIRMED / ALL FIXED** —
(1) eval/cookie ValueCallback registries leaked JNI globals across view teardown/shutdown; (2)+(4) the
reader-thread reentrancy self-deadlock class (a synchronous getCookie from any reader-dispatched upcall parked
the io thread 5 s and returned a wrong empty — fixed at the root by the upcall thread); (3) the failure latch
never drained the pending ValueCallback tables (in-flight callbacks never fired + globals leaked on helper
death/shutdown/renderer death — fixed by `drain_all_webview_callbacks`); (5) the eval degrade dropped its
retained callback unfired (now fires `onReceiveValue("null")`, the AOSP failure value); (6) `reader_fatal` left
getCookie waiters parked the full 5 s (now woken immediately); (7) `@JavascriptInterface` overloads collapsed to
one arbitrary Method (now arity-resolved per gin). Privacy-redaction and portability-scope dimensions: 0
findings. **Post-fix review (wf_d4c555fc): 4 alleged — 2 CONFIRMED + fixed** (A: never-driven-WebView bridge
leak — `notify_view_freed` early-returned before its cleanup; B: a stale queued ViewClosedDrain could fail a
fresh post-re-drive eval callback), **1 REFUTED with mechanism** (GC-finalize bypass of ViewClosedDrain —
unreachable in production because Eclipse's own per-view jobject global pins every constructed View; finalize
never runs, so the alleged primary-path zero-fire/leak does not exist), **1 unadjudicated** (skeptic died on a
session limit) → **re-adjudicated by the close-out pass (wf_ad0f573b): CONFIRMED (minor)** — the reader's
ViewClosed `drop_bridges_for` performed hidden scoped JNI attach/detach per bridge global on the supposedly
JNI-free io thread — **+ fixed** (the closed-view bridge drop moved to the upcall thread via the era-gated
`drop_bridges_for_view_closed`; queue order now also guarantees a BridgeCall received before ViewClosed still
finds its registry entry). Item C: stale protocol-v1 strings fixed tree-wide — the handshake log now binds the
negotiated `protocol=2`, module docs/comments updated, v1-as-frozen-subset statements deliberately kept.

*Owner live-verify record (dev-host main thread, 2026-07-10, log `/tmp/eclipse-webview-m4-test.log`, 52 lines):*
`timeout 180 ./target/release/eclipse __webview-test` → **EXIT=0** with the SUCCESS marker verbatim: `WebView
engine pipeline OK: internalLoadChanged upcalls 2/2 (state 0 @ 150ms, state 3 @ 150ms, http 200), frame 1024x768
237 distinct pixels, bridge round-trip OK, evaluateJavascript OK, honest UA OK, cookie set/get OK, cookie
callback OK, ViewClosed, helper exit 0, bound=5` — plus the CookieManager `bound=6` registration line. The
handshake line logs the negotiated `protocol=2` and `engine=cef/149.0.6+g0d0eeb6+chromium-149.0.7827.201`; the
helper resolved to the release binary; the driven page is the offline loopback page (the only `target=` lines
are `http://127.0.0.1:<port>` scheme+host and `<non-url>` bootstrap suppressions; the 237-pixel ink census is
the solid-color test page, not the roblox.com range). Privacy greps: 0 full-URL-shaped lines, 0
ROBLOSECURITY-shaped strings; bridge method names, cookie values, the UA and eval results are explicitly not
logged (`method names not logged` / `values not printed` / `UA value not printed` lines). /proc orphan scan
clean (zero eclipse-owned processes). An earlier identical run on the pre-close-out tree also passed EXIT=0
(state 0/3 @ 200 ms).

*Gate (final, post-close-out):* root `cargo fmt --all` / `cargo build --all-targets` / `cargo clippy
--all-targets --all-features -- -D warnings` / `cargo test` / `cargo build --release` ALL clean — **629 unit +
6 integ + 2 doctest, ZERO SKIP** (was 613+6+2 at M3; the `__webview-test` guard drove the full
natives→socket→helper→memfd pipeline through REAL ART and asserted the exact M4 marker); helper crate
(`CEF_PATH` = the reused M1 dist) fmt --check / build / clippy -D warnings / test / release clean — **23 + 15**
(was 17+11); root `Cargo.toml`/`Cargo.lock` verified cef-free (the always-run `root_lockfile_stays_cef_free` pin
+ independent grep).

*Carried:* (i) the M3 carried-to-M6 list stays carried unchanged — the ATL 2-arg `onPageStarted` vs AOSP 3-arg
reconciliation; pre-driven self-navigation silence (the driven-loads-only contract); input routing vs the
centered-fallback composite rect (the fallback rect deliberately does NOT capture input); challenge-rect
fidelity / `ResizeView` wiring at M6 per the recalibration recipe. (ii) TWO NEW owner-flagged unconfirmed
nuances from the close-out pass (both bounded, recorded not fixed): an in-flight eval callback of a finalized
(notify-path) view whose result raced the finalize is held until the helper-teardown drain; and the pre-existing
`pending_bridges` inventory-loss nuance on close+re-drive. (iii) M6 must overlay-validate the javascript:-URL
leak fix against the real challenge flow (the println is gone and the script routes to the engine — the
challenge's own `javascript:` usage is the live proof).

*Status:* M4 DONE + `__webview-test`-VERIFIED 2026-07-10, COMMITTED with this entry. Files:
`src/webview/{mod,proto,client}.rs`, `src/framework.rs`, `src/main.rs`, `tests/engine_milestones.rs`,
`crates/eclipse-webview/src/{main,engine,shared}.rs`, `crates/eclipse-webview/src/bin/drive.rs`,
`tools/framework-overlay/patch-framework.sh`, NEW
