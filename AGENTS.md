...
- **2026-06-14 — 🔑 SHADER-PACK REJECT ROOT-CAUSE HYPOTHESIS: THE APK IS BUNDLETOOL-TAMPERED / UNSIGNED. ⇐ START HERE NEXT SESSION (most promising lead, first-party/provisioning — NOT RE).**
  ...
  * DEAD END (proven): re-signing the merged APK with the vendored JDK21 `jarsigner` (V1, debug key) did NOT make ATL find a cert AND broke the boot (Java exception in lifecycle → `libroblox` SIGSEGV). Re-signing a tampered APK is not viable (wrong cert + zip rewrite breaks the loader's `.so`/asset offsets). The fix must be an **already-untampered, production-signed** APK, not a re-sign.
  * **NEXT (decisive test, needs the user):** obtain a **Roblox-production-signed x86_64 package** (APKMirror BUNDLE/`.apkm`, signature preserved — same source class Sober uses) and boot Eclipse against it. If the shader pack then opens → ROOT CAUSE CONFIRMED + render unblocked (provisioning fix, zero RE). If it still rejects → integrity is ruled out and the RBXS-check boundary returns. I cannot/should-not download Roblox APKs myself (large, external, user's responsibility per Sober's own docs) → this is the one open first-party lever and it's gated on the user supplying an untampered APK. The earlier "find the latest roblox apk" task should target **untampered + production-signed**, not just latest-version.
