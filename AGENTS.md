...
- **2026-06-14 — ❌ RETRACTED: "ACTIVE ANTI-TAMPER" WAS WRONG. The `~libroblox+0x_88a4d0` crash is the engine SIGSEGV-ing on a PENDING JAVA EXCEPTION during the lifecycle, NOT an anti-tamper handler. ⇐ START HERE NEXT SESSION (corrected).**
  ...
  **The shader-pack reject is SEPARATE from this crash** — it occurs in the normal boot that does NOT hit `0x_88a4d0` (reaches render). So the shader reject is still an unexplained in-engine RBXS-parse reject (the real boundary), now WITHOUT any anti-tamper framing.
  ⚠️ **CURRENT ENVIRONMENTAL BLOCKER (not a code bug — tree is clean at HEAD):** the GMS/Firebase `StreamCorruptedException` during `onCreate` is firing on EVERY boot right now (reverted code AND fresh app-data both crash at it), so the dev-host currently can't reach the shader stage. It is PRESENT-but-non-fatal in the older `eclipse-724.log` (timing put it on a background thread there). Likely tied to the **lost uncommitted `M src/framework.rs` WIP** that was present at session start and is now gone (not in stash/reflog/my commits) — it probably stubbed a GMS/Firebase/networking path (log marker `STRAWTOGRASP: GetFieldID(…, delegate, Ljava/net/SocketImpl;) returning NULL`). NEXT SESSION: first restore a boot that survives Firebase init (reconstruct that stub OR make `drive_application_lifecycle` clear+log+continue a non-fatal `onCreate` exception — but note that leaves onCreate incomplete, so the stub is better), THEN resume the shader-pack reject. The `StreamCorruptedException` is Firebase measurement deserializing an all-zeros Parcel from an ATL/GMS stub. CONFIRMED 2026-06-14: the throw is `android.os.Parcel.readSerializable` doing `new ObjectInputStream` over all-zeros bytes — but `readSerializable` is NOT in `vendor/atl`'s `Parcel.java` (it's compiled in the installed `api-impl.jar`), so an overlay patch needs the full reconstructed `Parcel` class (heavier than the Build/PowerManager/ActivityManager patches). Cleanest fixes, in order: (a) recover/recreate the lost WIP (it WAS the fix); (b) overlay-patch `Parcel.readSerializable` to `try { new ObjectInputStream(...) } catch (StreamCorruptedException) { return null; }`; (c) find+fix the upstream GMS stub that returns the empty/all-zeros Parcel. Re-boots crash DETERMINISTICALLY (idle AND under load), so it is NOT a winnable timing race in the committed tree — the committed tree alone cannot reach the shader stage without the WIP. NB: ATL's libcore has a `System.exit(10)` uncaught-exception handler (see `tools/framework-overlay/src/android/os/PowerManager.java` header), so any thread leaking this exception is process-fatal.
