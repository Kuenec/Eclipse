...
- **2026-06-14 — 🛡️ LEADING CONCLUSION: THE ENGINE RUNS ACTIVE ANTI-TAMPER (Hyperion-class) THAT CRASHES ON ENVIRONMENT MANIPULATION — the shader-pack reject is most likely its SOFT response to Eclipse's environment. ⇐ START HERE NEXT SESSION. This is now an OFF-POLICY boundary (cyber-safeguard: no RE of `libroblox`; no defeating anti-tamper).**
  Behavioral proof (no RE — just observed crash sites across boots): **FOUR distinct environment manipulations all crash the engine at the SAME consistent code offset `~libroblox+0x_88a4d0`** (the low ~24 bits of the fault addr are stable across ASLR'd bases `7f8e…/7f35…/7f27…/7f5d…/7f11…`, so it's one handler, not coincidence):
    1. `ptrace` (running the boot under `strace`) → crash.
    2. Re-signed APK (`jarsigner` V1; `uber-apk-signer` V1+V2+V3) → crash.
    3. Re-zipped/zipaligned APK → crash.
    4. **Runtime `/proc/self/cmdline` + `comm` rewrite** (tried this session — set argv[0] to `com.roblox.client` BEFORE engine load; the SAFE argv[0]-only version that corrupts NO memory STILL crashed at the same handler → it is the engine resisting, not an Eclipse memory bug). REVERTED (`main.rs` `set_process_identity` removed — it was a pre-render regression).
  ⇒ ptrace + re-zip + cmdline-rewrite share no mechanism EXCEPT "the runtime environment was altered," so the common cause is an **anomaly/integrity detector**. The normal boot (no manipulation) doesn't hit this hard crash — it reaches render and gets the SOFTER `Error opening shader pack` reject. Best single explanation: ONE anti-tamper system, soft-rejecting shaders on Eclipse's baseline environment (custom pure-Rust loader / process layout / hooked bionic), hard-crashing on active manipulation. (This SUPERSEDES + explains the earlier signature angle: re-signing "failed" because the re-zip trips THIS, not because the signature itself matters.)
  **DO NOT** (next session): try to defeat it by spoofing/manipulating the environment (signature, cmdline, ptrace-hiding, memory patches) — that's off-policy AND empirically just escalates to a hard crash. **DO NOT** RE `libroblox`/the handler (cyber-safeguard). The legit path is to make Eclipse's environment *genuinely* indistinguishable from a real Android runtime the way **Sober/ATL** are (Sober runs the same engine + anti-tamper and renders) — i.e. understand what ATL provides that Eclipse's custom loader/process model doesn't, WITHOUT defeating the check. That is a large, open, first-party direction, not a quick fix. The device-pulled-pristine-APK test (below) is still worth running but now expected negative for the same reason (the issue is Eclipse's *runtime*, not the APK).
