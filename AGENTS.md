...
- **2026-06-14 — ✅✅ SHADER-PACK REJECT ROOT-CAUSED AND FIXED — IT WAS A FIRST-PARTY ECLIPSE BUG, NOT "internal/gated/off-policy". THE ENGINE NOW LOADS THE SHADER PACK, CREATES ITS RenderView, AND REACHES THE LuaApp LANDING SCREEN. ⇐ START HERE NEXT SESSION. (commit `ac5feba`)**
  ...
  **LIVE-CONFIRMED on the dev host** (`/tmp/boot_fd.log`, patched framework, EXIT=124 clean 240 s, 1311 lines): `Loaded 3470 shaders from pack vulkan_mobile variant default (17967131 bytes)` (line 1039) · `RenderView created[1]` (line 1007, was NULL) · `Vulkan: swapchain images 3 … size 800x600` · `SurfaceController::RenderJob` running · `onDataModelNotification() type:APP_READY data:Landing` (line 1201) · `LuaAppStarterScript` running. The only remaining errors are HTTP **403** on `user-heartbeats-api/pulse` — expected (no authenticated user session; not logged in), NOT a render failure.
  ...
  ⚠️ **NEW BLOCKER = INPUT NOT FORWARDED (the next layer, owner-reported 2026-06-14): clicking the rendered UI does nothing.** Confirmed first-party root cause(s): `src/graphics.rs` `handle_primary_press`/`handle_primary_release` early-return when `self.renderer` is `None` — but the renderer is DROPPED at the present-loop handoff (`src/graphics.rs:353`), so EVERY post-handoff click is silently dropped (live log: zero "pointer MotionEvent dispatched" lines despite the owner clicking). Secondary: the existing pointer path hit-tests Eclipse's OWN laid-out View tree (designed for the multitouch.test app), which cannot resolve the engine-drawn Lua UI — it must instead dispatch a raw-coordinate `MotionEvent` to the engine's content view `com.roblox.client.RBXSurfaceView` (extends `android.view.SurfaceView`, already tracked in `view_registry`, framework.rs ~7086) via `dispatchTouchEvent`. Likely prerequisite: `hasSystemFeature(android.hardware.touchscreen)` returns "not implemented yet" (logged 4×) — the engine may disable touch; needs a real overlay/native fix. The engine has NO AInputQueue NDK natives (native_provider ~3607), so the Java `dispatchTouchEvent → RBXSurfaceView.onTouchEvent → native` path is the engine's actual input route. Investigation + fix design: workflow `eclipse-input-forwarding-investigation` (in flight). Implement + live-validate (orchestrator).
