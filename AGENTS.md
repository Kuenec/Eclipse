...
- **2026-06-14 — ✅✅✅ HOST INPUT IS FORWARDED — CLICKS REACH ROBLOX AND DRIVE THE UI. Tapping "Sign In" navigated the app Landing → LoginV2. ⇐ START HERE NEXT SESSION.**
  ...
  **LIVE-CONFIRMED** (`/tmp/boot_oti.log`, patched framework): synthetic tap at (400,390)=Sign In → `onDataModelNotification type:APP_READY data:LoginV2` + `ActivityNativeMain onAppReady: LoginV2` (Landing→Login nav); the owner's subsequent clicks all log `consumed=true`. Gate green (fmt/clippy/567 unit + 4 integ + 2 doctest/release).
  ...
  ⇒ Full login typing needs Eclipse to implement ATL's **EditText native text backing** — a SUBSYSTEM (own phase), NOT a quick bind. Scoped components: (1) `native_setText(long,String)` is ALREADY bound (`widget_native_set_text`); `native_getText(long)→String` is the unbound frontier — and it is the FIRST **value-returning** framework native, which needs NEW machinery: the `EnvUnowned::with_env(...).resolve::<P>()` path requires `T: Default`, and raw `jstring` (`*mut _jobject`) is not `Default` — so use a `#[repr(transparent)]` newtype over `jstring` with a null `Default` (ABI-identical), or `resolve_with`. (2) text storage already exists (`view_registry` `text` field + `add_text_watcher`/`retain_text_watchers` — watchers are already retained). (3) a key→text editing path: route a typed char to the ACTIVE EditText (focus-tracking — which EditText is the engine's current input field; the `RbxKeyboard` EditText is the candidate), append/backspace its stored text, and FIRE the retained `TextWatcher` (`beforeTextChanged`/`onTextChanged`/`afterTextChanged` via JNI) so the engine observes the change. The `RBXSurfaceView.dispatchKeyEvent` route stays correct for IN-GAME keys; the LOGIN field is this separate EditText path.
