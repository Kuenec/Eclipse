...
- **2026-06-14 — ✅✅✅ HOST INPUT IS FORWARDED — CLICKS REACH ROBLOX AND DRIVE THE UI. Tapping "Sign In" navigated the app Landing → LoginV2. ⇐ START HERE NEXT SESSION.**
  ...
  **LIVE-CONFIRMED** (`/tmp/boot_oti.log`, patched framework): synthetic tap at (400,390)=Sign In → `onDataModelNotification type:APP_READY data:LoginV2` + `ActivityNativeMain onAppReady: LoginV2` (Landing→Login nav); the owner's subsequent clicks all log `consumed=true`. Gate green (fmt/clippy/567 unit + 4 integ + 2 doctest/release).
  ...
  🔬 **VISIBLE-TEXT — IN-POLICY APPROACHES EXHAUSTED (2026-06-14).** Now firing the FULL `TextWatcher` contract (`beforeTextChanged` → `onTextChanged` → `afterTextChanged`, with correct per-keystroke deltas + a SpannableStringBuilder Editable) — all fire cleanly and the engine still VALIDATES the text (Next stays enabled) — but the field is still not drawn. ⇒ the engine's field-text RENDERING is independent of the TextWatcher (it reads from a non-TextWatcher source). The remaining levers are OFF-POLICY or large: (a) RE the engine's Lua-TextBox display binding (off-policy), or (b) composite the Android `EditText`'s own text draw over the engine's GL surface (a new rendering feature — Eclipse's no-GTK framework draws no Android views). Functional typing is DONE; visible typing needs one of those phases.
