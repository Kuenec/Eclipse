...
- **2026-06-14 — ✅✅✅ HOST INPUT IS FORWARDED — CLICKS REACH ROBLOX AND DRIVE THE UI. Tapping "Sign In" navigated the app Landing → LoginV2. ⇐ START HERE NEXT SESSION.**
  ...
  **LIVE-CONFIRMED** (`/tmp/boot_oti.log`, patched framework): synthetic tap at (400,390)=Sign In → `onDataModelNotification type:APP_READY data:LoginV2` + `ActivityNativeMain onAppReady: LoginV2` (Landing→Login nav); the owner's subsequent clicks all log `consumed=true`. Gate green (fmt/clippy/567 unit + 4 integ + 2 doctest/release).
  ...
  ⇒ Full login typing needs Eclipse to implement ATL's **EditText native text backing**: `native_getText`/`native_setText` (store the field's text in an Eclipse registry peer) + a key→text editing path (a dispatched `KeyEvent`/its `unicodeValue` must mutate that text so `getText()` returns it). The `RBXSurfaceView.dispatchKeyEvent` route is correct for IN-GAME keys; the LOGIN field is a separate EditText-text-storage subsystem (substantial — warrants its own phase, likely a workflow to design the text peer + editing). NOT just binding two natives.
